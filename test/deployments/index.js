			...
			(['goerli', 'rinkeby', 'ropsten'].indexOf(network) > -1 ? describe.skip : describe)(
				...
				() => {
					...
					beforeEach(() => {
						...
						contracts = {
							DPassive: getContract({ source: 'DPassive', target: 'ProxyERC20' }),
								...
								it(`${name} has valid staking and reward tokens`, async () => {
									...
									for (const { token, tokenAddress } of tokens) {
										...
										// try and compare the name
											...
											if (token === 'DPassive' || token === 'ProxyERC20') {
												assert.strictEqual(tokenName, 'dPassive Native Token');
					...
					describe('synths.json', () => {
						...
						it(`The number of available synths in DPassive matches the number of synths in the JSON file: ${synths.length}`, async () => {
							const availableSynths = await contracts.DPassive.methods
						...
						synths.forEach(({ name, inverted, feed, index }) => {
							describe(name, () => {
								it('DPassive has the synth added', async () => {
									const foundSynth = await contracts.DPassive.methods
