...
async function checkRates({ network, deploymentPath }) {
	...
	const rateInvalid = await DPassive.anySynthOrDPSRateIsInvalid();
...
async function simulateExchangeRates({ network, deploymentPath }) {
	...
	const additionalKeys = ['DPS', 'ETH'].map(toBytes32); // The Depot uses the key "ETH" as opposed to "dETH" for its ether price
