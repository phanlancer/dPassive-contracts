...
describe('publish scripts', () => {
	...
	describe('integrated actions test', () => {
		describe('when deployed', () => {
			...
			describe('when ExchangeRates has prices DPS $0.30 and all synths $1', () => {
				beforeEach(async () => {
					...
					const answersToSet = [{ asset: 'DPS', rate: 0.3 }].concat(
				...
				describe('when transferring 100k DPS to user1', () => {
					beforeEach(async () => {
						// transfer DPS to first account
						...
						describe('when user1 exchange 1000 dUSD for dBTC', () => {
							...
							describe('when user1 burns 10 dUSD', () => {
								beforeEach(async () => {
									...
									// burn
									await DPassive.methods.burnSynths(web3.utils.toWei('10')).send({
										from: accounts.first.public,
										gas: gasLimit,
										gasPrice,
									});
							...
							describe('when the aggregator has a price', () => {
								...
								describe('when DPassive.anySynthOrDPSRateIsInvalid() is invoked', () => {
									it('then it returns false as expected', async () => {
										const response = await DPassive.methods.anySynthOrDPSRateIsInvalid().call();
										assert.strictEqual(response, false, 'anySynthOrDPSRateIsInvalid must be false');
									});
								});
							});
						});
						...
						it('and the resolver has all the addresses inside', async () => {
							...
							const responses = await Promise.all(
								[
									...
									callMethodWithRetry(
										AddressResolver.methods.getAddress(dps.toBytes32(contractName))
					...
					describe('when re-deployed', () => {
						...
						beforeEach(async () => {
							...
							const existingExchanger = await callMethodWithRetry(
								AddressResolver.methods.getAddress(dps.toBytes32('Exchanger'))
							);
