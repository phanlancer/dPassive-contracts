...
contract('BaseRewardEscrowV2', async accounts => {
	...
	beforeEach(async () => {
		({ mocks, resolver } = await prepareSmocks({
			contracts: ['FeePool', 'Issuer', 'DPassive'],
	...
	describe('Creating vesting Schedule', async () => {
		describe('When appending vesting entry via feePool', async () => {
			...
			it('should revert appending a vesting entry from account1', async () => {
				// Transfer of DPS to the escrow must occur before creating an entry
				mocks['DPassive'].smocked.balanceOf.will.return.with(parseEther('10'));
			...
			it('should revert appending a vesting entry with a zero amount', async () => {
				// Transfer of DPS to the escrow must occur before creating an entry
				mocks['DPassive'].smocked.balanceOf.will.return.with(parseEther('1'));
			...
			it('should revert appending a vesting entry if there is not enough DPS in the contracts balance', async () => {
				// Transfer of DPS to the escrow must occur before creating an entry
				mocks['DPassive'].smocked.balanceOf.will.return.with(parseEther('1'));
			...
			describe('When successfully appending new escrow entry for account 1 with 10 DPS', () => {
				...
				it('Account 1 should have balance of 10 DPS', async () => {
					assert.bnEqual(await baseRewardEscrowV2.balanceOf(account1), escrowAmount);
				});
				it('totalEscrowedBalance of the contract should be 10 DPS', async () => {
				...
				describe('When 6 months has passed', () => {
					...
					it('then the vesting entry has 0 dps claimable', async () => {
	...
	describe('Creating a new escrow entry by approval', async () => {
		...
		beforeEach(async () => {
			// approve rewardEscrow to spend DPS
			mocks['DPassive'].smocked.allowance.will.return.with(parseEther('10'));

			// stub transferFrom
			mocks['DPassive'].smocked.transferFrom.will.return.with(true);
		...
		describe('when successfully creating a new escrow entry for acount 1', () => {
			...
			it('Account1 should have balance of 10 DPS', async () => {
				assert.bnEqual(await baseRewardEscrowV2.balanceOf(account1), escrowAmount);
			});
			it('Account1 should have totalVested Account Balance of 0', async () => {
				assert.bnEqual(await baseRewardEscrowV2.totalVestedAccountBalance(account1), new BN(0));
	...
	describe('Vesting', () => {
		...
		beforeEach(async () => {
			// Mock DPS ERC20
			({ token: mockedDPassive } = await mockToken({
				accounts,
				name: 'DPassive',
				symbol: 'DPS',
			}));

			// replace dpassive on resolver
			const newResolver = await artifacts.require('AddressResolver').new(owner);

			await newResolver.importAddresses(
				['DPassive', 'FeePool', 'Issuer'].map(toBytes32),
				[mockedDPassive.address, feePoolAccount, mocks['Issuer'].address],
				{ from: owner }
			);
			...
			// Transfer of DPS to the escrow must occur before creating a vestinng entry
			await mockedDPassive.transfer(baseRewardEscrowV2.address, toUnit('1000'), {
				from: owner,
			});
		});
		describe('Vesting of vesting entry after 6 months (before escrow ends)', () => {
			...
			let escrowAmount, timeElapsed, entryID, claimableDPS;
			beforeEach(async () => {
			...
			it('should vest 0 amount if entryID does not exist for user', async () => {
				...
				// Check user has no vested DPS
				assert.bnEqual(await mockedDPassive.balanceOf(account1), toUnit('0'));

				// Check rewardEscrow does not have any DPS
				assert.bnEqual(await mockedDPassive.balanceOf(baseRewardEscrowV2.address), escrowAmount);
			...
			it('should vest and transfer 0 DPS from contract to the user', async () => {
				claimableDPS = await baseRewardEscrowV2.getVestingEntryClaimable(account1, entryID);

				const escrowBalanceBefore = await mockedDPassive.balanceOf(baseRewardEscrowV2.address);
				...
				// Check user has the 0 vested DPS
				assert.bnEqual(await mockedDPassive.balanceOf(account1), 0);

				// Check rewardEscrow contract has same amount of DPS
				assert.bnEqual(
					await mockedDPassive.balanceOf(baseRewardEscrowV2.address),
					escrowBalanceBefore
				);
		...
		describe('When vesting after escrow ended', () => {
			...
			it('should vest and transfer all the dps to the user', async () => {
				await baseRewardEscrowV2.vest([entryID], {
					from: account1,
				});

				// Check user has all their vested DPS
				assert.bnEqual(await mockedDPassive.balanceOf(account1), escrowAmount);

				// Check rewardEscrow does not have any DPS
				assert.bnEqual(await mockedDPassive.balanceOf(baseRewardEscrowV2.address), toUnit('0'));
			});
		...
		describe('Vesting multiple vesting entries', () => {
			...
			describe('When another user (account 1) vests all their entries', () => {
				it('should vest all entries and transfer dps to the user', async () => {
					await baseRewardEscrowV2.vest([entryID1, entryID2, entryID3], {
						from: account2,
					});

					// Check account1 has no DPS in their balance
					assert.bnEqual(await mockedDPassive.balanceOf(account1), toUnit('0'));

					// Check account2 has no DPS in their balance
					assert.bnEqual(await mockedDPassive.balanceOf(account2), toUnit('0'));

					// Check rewardEscrow has all the DPS
					assert.bnEqual(
						await mockedDPassive.balanceOf(baseRewardEscrowV2.address),
						toUnit('1000')
					);
			...
			it('should vest all entries and transfer dps from contract to the user', async () => {
				await baseRewardEscrowV2.vest([entryID1, entryID2, entryID3], {
					from: account1,
				});

				// Check user has all their vested DPS
				assert.bnEqual(await mockedDPassive.balanceOf(account1), toUnit('1000'));
			...
			it('should vest all entries and ignore duplicate attempts to vest same entries again', async () => {
				...
				// Check user has same amount of DPS
				assert.bnEqual(await mockedDPassive.balanceOf(account1), toUnit('1000'));
		...
		describe('Vesting multiple vesting entries with different duration / end time', () => {
			...
			describe('when the first two entrys are vestable', () => {
				...
				it('should vest only first 2 entries and transfer dps from contract to the user', async () => {
					await baseRewardEscrowV2.vest([entryID1, entryID2, entryID3], {
						from: account1,
					});
					...
					assert.bnEqual(
						await mockedDPassive.balanceOf(account1),
						escrowAmount1.add(escrowAmount2)
					);

					// Check rewardEscrow has remaining entry3 amount
					assert.bnEqual(await mockedDPassive.balanceOf(baseRewardEscrowV2.address), escrowAmount3);
				});
				...
				it('should vest and update totalEscrowedAccountBalance', async () => {
					...
					// This account should have any 500 DPS escrowed
					escrowedAccountBalance = await baseRewardEscrowV2.totalEscrowedAccountBalance(account1);
				...
				it('should vest entryID1 and entryID2 and ignore duplicate attempts to vest same entries again', async () => {
					...
					// Check user have vested escrowAmount1 and escrowAmount2 DPS
					assert.bnEqual(
						await mockedDPassive.balanceOf(account1),
						escrowAmount1.add(escrowAmount2)
					);

					// Check rewardEscrow does has escrowAmount3 DPS
					assert.bnEqual(await mockedDPassive.balanceOf(baseRewardEscrowV2.address), escrowAmount3);
					...
					// Check rewardEscrow has same escrowAmount3 DPS
					assert.bnEqual(await mockedDPassive.balanceOf(baseRewardEscrowV2.address), escrowAmount3);
				});
			});
		});
	...
	describe('Read Vesting Schedule', () => {
		...
		beforeEach(async () => {
			// Transfer of DPS to the escrow must occur before creating a vestinng entry
			mocks['DPassive'].smocked.balanceOf.will.return.with(parseEther('1000'));
