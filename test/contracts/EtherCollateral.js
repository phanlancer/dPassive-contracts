...
contract('EtherCollateral', async accounts => {
	...
	const [dETH, ETH, DPS] = ['dETH', 'ETH', 'DPS'].map(toBytes32);
	...
	const issueSynthdUSD = async (issueAmount, receiver) => {
	...
	const depositUSDInDepot = async (synthsToDeposit, depositor) => {
		...
		// Get dUSD from Owner
		await issueSynthdUSD(synthsToDeposit, depositor);

		// Approve Transaction
		await dUSDSynth.approve(depot.address, synthsToDeposit, { from: depositor });
	...
	const calculateLoanFeedUSD = async feesInETH => {
	...
	const updateRatesWithDefaults = async () => {
		...
		// Depot requires DPS and ETH rates
	...
	before(async () => {
		// Mock DPS, dUSD and dETH
		[{ token: dpassive }, { token: dUSDSynth }, { token: dETHSynth }] = await Promise.all([
			mockToken({ accounts, name: 'DPassive', symbol: 'DPS' }),
	...
	describe('when opening a Loan', async () => {
		...
		describe('then create loan and', async () => {
			...
			const expecteddETHLoanAmount = calculateLoanAmount(tenETH);
			...
			it('increase the totalIssuedSynths', async () => {
				assert.bnEqual(await etherCollateral.totalIssuedSynths(), expecteddETHLoanAmount);
			});
			it('emit a LoanCreated event', async () => {
				assert.eventEqual(openLoanTransaction, 'LoanCreated', {
					account: address1,
					loanID: 1,
					amount: expecteddETHLoanAmount,
				});
			});
			...
			it('store the synthLoan.loanAmount', async () => {
				const synthLoan = await etherCollateral.getLoan(address1, loanID);
				assert.bnEqual(synthLoan.loanAmount, expecteddETHLoanAmount);
			});
			...
			it('add the loan issue amount to creators balance', async () => {
				const dETHBalance = await dETHSynth.balanceOf(address1);
				assert.bnEqual(dETHBalance, expecteddETHLoanAmount);
			});
			...
			describe('when opening a second loan against address1', async () => {
				...
				const expecteddETHLoanAmount = calculateLoanAmount(fiveThousandETH);
				...
				it('then increase the totalIssuedSynths', async () => {
					assert.bnEqual(
						...
						totalIssuedSynthsBefore.add(expecteddETHLoanAmount)
					);
				});
				...
				describe('when opening a third loan against address2', async () => {
					...
					const expecteddETHLoanAmount = calculateLoanAmount(threeNintyETH);
					...
					it('then increase the totalIssuedSynths', async () => {
						assert.bnEqual(
							await etherCollateral.totalIssuedSynths(),
							totalSupplyBefore.add(expecteddETHLoanAmount)
						);
					});
					...
					describe('when closing the first loan of address1', async () => {
						...
						beforeEach(async () => {
							...
							// expectedFeedUSD = await calculateLoanFeedUSD(expectedFeeETH);
		...
		describe('when closing a Loan', async () => {
			...
			describe('check conditions', async () => {
				...
				it('when dETH balance is less than loanAmount, then it reverts', async () => {
					...
					await dETHSynth.transfer(owner, toUnit('4'), { from: address1 });
				...
				describe('potential blocking conditions', () => {
					beforeEach(async () => {
						...
						await depositUSDInDepot(oneThousanddUSD, depotDepositor);
					});
			...
			describe('then it closes the loan and', async () => {
				...
				it('increase the FeePool dUSD balance', async () => {
					assert.bnEqual(await dUSDSynth.balanceOf(FEE_ADDRESS), expectedFeedUSD);
				});
				...
				it('Depots totalSellableDeposits has reduced by the expected fees dUSD amount', async () => {
					...
					assert.bnEqual(
						...
						totalSellableDepositsBefore.sub(expectedFeedUSD)
					);
				});
				...
				it('decrease the dUSD balance in Depot', async () => {
					const expectedBalance = oneThousanddUSD.sub(expectedFeedUSD);
					assert.bnEqual(await dUSDSynth.balanceOf(depot.address), expectedBalance);
				});
	...
	describe('when loanLiquidation is opened', async () => {
		...
		it('then alice has a dETH loan balance', async () => {
			assert.bnEqual(await dETHSynth.balanceOf(alice), expecteddETHLoanAmount);
		});
		describe('when loanLiquidation is open', () => {
			...
			describe('when chad has some dETH and alice has her dETH still', () => {
				...
				describe('and chad liquidates alices dETH loan for her ETH', async () => {
					...
					it('then alice dETH balance is still intact', async () => {
						assert.ok(await dETHSynth.balanceOf(alice), expecteddETHLoanAmount);
					});
					it('then chads dETH balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await dETHSynth.balanceOf(chad), 0);
					});
			...
			describe('when bob has some dETH', () => {
				beforeEach(async () => {
					await dETHSynth.transfer(bob, await dETHSynth.balanceOf(alice), { from: alice });
				});
				describe('and bob liquidates alices dETH loan for her ETH', async () => {
					...
					it('then alice dETH balance is 0 (because she transfered it to bob)', async () => {
						assert.ok(await dETHSynth.balanceOf(alice), 0);
					});
					it('then bobs dETH balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await dETHSynth.balanceOf(bob), 0);
					});
					it('then emits a LoanLiquidated event', async () => {
						assert.eventsEqual(
							liquidateLoanTransaction,
