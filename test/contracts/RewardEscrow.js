...
contract('RewardEscrow', async accounts => {
	...
	let rewardEscrow, dpassive, feePool;

	// Run once at beginning - snapshots will take care of resetting this before each test
	before(async () => {
		// Mock DPS
		({ token: dpassive } = await mockToken({ accounts, name: 'DPassive', symbol: 'DPS' }));
	...
	describe('Constructor & Settings ', async () => {
		it('should set dpassive on contructor', async () => {
			const dpassiveAddress = await rewardEscrow.dpassive();
			assert.equal(dpassiveAddress, dpassive.address);
		});
		...
		it('should allow owner to set dpassive', async () => {
			await rewardEscrow.setDPassive(ZERO_ADDRESS, { from: owner });
			const dpassiveAddress = await rewardEscrow.dpassive();
			assert.equal(dpassiveAddress, ZERO_ADDRESS);
		});
	...
	describe('Functions', async () => {
		...
		describe('Vesting Schedule Writes', async () => {
			it('should not create a vesting entry with a zero amount', async () => {
				...
				await dpassive.transfer(rewardEscrow.address, toUnit('1'), {
					from: owner,
				});
			...
			it('should not create a vesting entry if there is not enough DPS in the contracts balance', async () => {
				// Transfer of DPS to the escrow must occur before creating an entry
				await dpassive.transfer(rewardEscrow.address, toUnit('1'), {
					from: owner,
				});
		...
		describe('Vesting Schedule Reads ', async () => {
			beforeEach(async () => {
				...
				await dpassive.transfer(rewardEscrow.address, toUnit('6000'), {
					from: owner,
				});
			...
			it('should append a vesting entry and increase the contracts balance', async () => {
				const balanceOfRewardEscrow = await dpassive.balanceOf(rewardEscrow.address);
		...
		describe('Vesting', async () => {
			...
			it('should vest and transfer dps from contract to the user', async () => {
				await rewardEscrow.vest({ from: account1 });

				// Check user has all their vested DPS
				assert.bnEqual(await dpassive.balanceOf(account1), toUnit('6000'));

				// Check rewardEscrow does not have any DPS
				assert.bnEqual(await dpassive.balanceOf(rewardEscrow.address), toUnit('0'));
			});
		...
		describe('Stress Test', () => {
			it('should not create more than MAX_VESTING_ENTRIES vesting entries', async () => {
				...
				// Transfer of DPS to the escrow must occur before creating an entry
				await dpassive.transfer(rewardEscrow.address, toUnit('260'), {
					from: owner,
				});
			...
			it('should be able to vest 52 week * 5 years vesting entries', async () => {
				...
				// Check user has all their vested DPS
				assert.bnEqual(await dpassive.balanceOf(account1), toUnit('260'));
			...
			it('should be able to read an accounts schedule of 5 vesting entries', async () => {
				// Transfer of DPS to the escrow must occur before creating an entry
				await dpassive.transfer(rewardEscrow.address, toUnit('5'), {
					from: owner,
				});
		...
		describe('Transfering', async () => {
			it('should not allow transfer of dpassive in escrow', async () => {
				...
				await assert.revert(
					dpassive.transfer(account2, toUnit('1000'), {
						from: account1,
					})
				);
			});
