...
contract('FeePool', async accounts => {
	...
	describe('restricted methods', () => {
		it('appendAccountIssuanceRecord() cannot be invoked directly by any account', async () => {
			await onlyGivenAddressCanInvoke({
				...
				reason: 'Issuer and DPassiveState only',
			});
		});
	});
	...
	describe('when the issuanceRatio is 0.2', () => {
		...
		it('should correctly calculate the totalFeesAvailable for multiple periods', async () => {
			...
			await dpassive.issueSynths(amount1, { from: owner });
		...
		describe('FeeClaimablePenaltyThreshold', async () => {
			...
			it('should be no penalty if issuance ratio is less than target ratio', async () => {
				await dpassive.issueMaxSynths({ from: owner });

				// Increase the price so we start well and truly within our 20% ratio.
				const newRate = (await exchangeRates.rateForCurrency(DPS)).add(web3.utils.toBN('1'));
				const timestamp = await currentTime();
			...
			it('should correctly calculate the 10% buffer for penalties at specific issuance ratios', async () => {
				...
				// Increase the price so we start well and truly within our 20% ratio.
				const newRate = (await exchangeRates.rateForCurrency(DPS)).add(
				...
				// Start from the current price of dpassive and slowly decrease the price until
				// we hit almost zero. Assert the correct penalty at each point.
				while ((await exchangeRates.rateForCurrency(DPS)).gt(step.mul(web3.utils.toBN('2')))) {
					const ratio = await dpassive.collateralisationRatio(owner);
					...
					// Bump the rate down.
					const newRate = (await exchangeRates.rateForCurrency(DPS)).sub(step);
					const timestamp = await currentTime();
					await exchangeRates.updateRates([DPS], [newRate], timestamp, {
						from: oracle,
					});
			...
			it('should revert when users try to claim fees with > 10% of threshold', async () => {
				...
				// But if the price of DPS decreases by 15%, we will lose all the fees.
