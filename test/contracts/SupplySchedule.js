...
contract('SupplySchedule', async accounts => {
	...
	const [, owner, dpassive, account1, account2] = accounts;

	let supplySchedule, dpassiveProxy, decayRate;
	...
	beforeEach(async () => {
		...
		dpassiveProxy = await setupContract({ accounts, contract: 'ProxyERC20' });

		await supplySchedule.setDPassiveProxy(dpassiveProxy.address, { from: owner });
		await dpassiveProxy.setTarget(dpassive, { from: owner });
	...
	it('only expected functions should be mutative', () => {
		ensureOnlyExpectedMutativeFunctions({
			...
			ignoreParents: ['Owned'],
			expected: ['recordMintEvent', 'setMinterReward', 'setDPassiveProxy'],
		});
	});
	...
	describe('linking dpassive', async () => {
		it('should have set dpassive proxy', async () => {
			const dpassiveProxy = await supplySchedule.dpassiveProxy();
			assert.equal(dpassiveProxy, dpassiveProxy);
		});
		it('should revert when setting dpassive proxy to ZERO_ADDRESS', async () => {
			await assert.revert(supplySchedule.setDPassiveProxy(ZERO_ADDRESS, { from: owner }));
		});

		it('should emit an event when setting dpassive proxy', async () => {
			const txn = await supplySchedule.setDPassiveProxy(account2, { from: owner });

			assert.eventEqual(txn, 'DPassiveProxyUpdated', {
				newAddress: account2,
			});
		});

		it('should disallow a non-owner from setting the dpassive proxy', async () => {
			await onlyGivenAddressCanInvoke({
				fnc: supplySchedule.setDPassiveProxy,
				args: [account2],
				address: owner,
				accounts,
			});
	...
	describe('functions and modifiers', async () => {
		...
		describe('mintable supply', async () => {
			...
			async function checkMintedValues(
				...
				// call updateMintValues to mimic dpassive issuing tokens
			...
			describe('setting weekCounter and lastMintEvent on supplySchedule to week 39', async () => {
				...
				beforeEach(async () => {
					...
					// setup new instance
					await instance.setDPassiveProxy(dpassiveProxy.address, { from: owner });
					await dpassiveProxy.setTarget(dpassive, { from: owner });
				});
