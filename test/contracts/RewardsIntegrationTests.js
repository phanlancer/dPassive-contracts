...
contract('Rewards Integration Tests', accounts => {
	...
	// CURRENCIES
	const [dUSD, dAUD, dEUR, dBTC, DPS, iBTC, dETH, ETH] = [
		'dUSD',
		'dAUD',
		'dEUR',
		'dBTC',
	...
	const synthKeys = [dUSD, dAUD, dEUR, dBTC, iBTC, dETH, ETH];
	...
	const updateRatesWithDefaults = async () => {
		...
		await exchangeRates.updateRates(
			[dAUD, dEUR, DPS, dBTC, iBTC, dETH, ETH],
	...
	const fastForwardAndCloseFeePeriod = async () => {
		...
		// alleviated an issues with the tests flaking
	...
	before(async function() {
		...
		({
			...
			accounts,
			synths: ['dUSD', 'dAUD', 'dEUR', 'dBTC', 'iBTC', 'dETH'],
			contracts: [
				'AddressResolver',
				...
				'DebtCache',
				'RewardEscrowV2',
				'RewardsDistribution', // required for DPassive.mint()
				'SupplySchedule',
				'DPassive',
				'SystemSettings',
				'CollateralManager',
		...
		MINTER_DPS_REWARD = await supplySchedule.minterReward();
	...
	beforeEach(async () => {
		...
		// Assign 1/3 of total DPS to 3 accounts
		const dpsTotalSupply = await dpassive.totalSupply();
		const thirdOfDPS = third(dpsTotalSupply);

		await dpassive.transfer(account1, thirdOfDPS, { from: owner });
		await dpassive.transfer(account2, thirdOfDPS, { from: owner });
		await dpassive.transfer(account3, thirdOfDPS, { from: owner });

		// Get the DPS mintableSupply
		periodOneMintableSupplyMinusMinterReward = (await supplySchedule.mintableSupply()).sub(
			MINTER_DPS_REWARD
		);

		// Mint the staking rewards
		await dpassive.mint({ from: deployerAccount });
	...
	describe('3 accounts with 33.33% DPS all issue MAX and claim rewards', async () => {
		...
		beforeEach(async () => {
			...
			await dpassive.issueMaxSynths({ from: account2 });
			await dpassive.issueMaxSynths({ from: account3 });
		});
		...
		it('should show the totalRewardsAvailable in the claimable periods 1 & 2', async () => {
			...
			for (let i = 0; i <= CLAIMABLE_PERIODS - 1; i++) {
				...
				// Get the DPS mintableSupply - the minter reward of 200 DPS
				mintedRewardsSupply = (await supplySchedule.mintableSupply()).sub(MINTER_DPS_REWARD);
				// console.log('mintedRewardsSupply', mintedRewardsSupply.toString());
				// Mint the staking rewards
				await dpassive.mint({ from: owner });
		...
		it('should mint DPS for the all claimable fee periods then all 3 accounts claim at the end of the claimable period', async () => {
			let mintedRewardsSupply;
			// We are currently in the 2nd week, close it and the next
			for (let i = 0; i <= CLAIMABLE_PERIODS - 1; i++) {
				// console.log('Close Fee Period', i);
		...
		it('should rollover the unclaimed DPS rewards', async () => {
		...
		it('should rollover the unclaimed DPS rewards on week over 2 terms', async () => {
			for (let i = 0; i <= 2; i++) {
				await fastForwardAndCloseFeePeriod();
				// FastForward a bit to be able to mint
				await fastForwardAndUpdateRates(MINUTE);
		...
		it('should rollover the partial unclaimed DPS rewards', async () => {
			// await logFeePeriods();
		...
		it('should allow a user to leave the system and return and still claim rewards', async () => {
			...
			const burnableTotal = await dpassive.debtBalanceOf(account1, dUSD);
			await dpassive.burnSynths(burnableTotal, { from: account1 });
		...
		it('should allocate correct DPS rewards as others leave the system', async () => {
			// Close Fee Period
			...
			// Get the DPS mintableSupply for period 2
	...
	describe('Exchange Rate Shift tests', async () => {
		it('should assign accounts (1,2,3) to have (40%,40%,20%) of the debt/rewards', async () => {
			// Account 1&2 issue 10K USD and exchange in dBTC each, holding 50% of the total debt.
			await dpassive.issueSynths(tenK, { from: account1 });
			await dpassive.issueSynths(tenK, { from: account2 });

			await dpassive.exchange(dUSD, tenK, dBTC, { from: account1 });
			await dpassive.exchange(dUSD, tenK, dBTC, { from: account2 });
			...
			// Account 3 (enters the system and) mints 10K dUSD (minus half of an exchange fee - to balance the fact
			// that the other two holders have doubled their dBTC holdings) and should have 20% of the debt not 33.33%
			const potentialFee = exchangeFeeIncurred(toUnit('10000'));
			await dpassive.issueSynths(tenK.sub(half(potentialFee)), { from: account3 });

			// Get the DPS mintableSupply for week 2
			...
			const { amountReceived } = await exchanger.getAmountsForExchange(tenK, dUSD, dBTC);
			await dpassive.exchange(dBTC, amountReceived, dUSD, { from: account1 });
			await dpassive.exchange(dBTC, amountReceived, dUSD, { from: account2 });
			...
			// // Account 1 exchanges all dBTC back to dUSD
			// const acc1dBTCBalance = await dBTCContract.balanceOf(account1, { from: account1 });
			// await dpassive.exchange(dBTC, acc1dBTCBalance, dUSD, { from: account1 });
			// const amountAfterExchange = await feePool.amountReceivedFromExchange(acc1dBTCBalance);
			...
			// await dpassive.burnSynths(amountAfterExchangeInUSD, { from: account1 });

			// // Get the DPS mintableSupply for week 3
			...
			// await dpassive.issueSynths(twentyK, { from: account1 });

			// // Get the DPS mintableSupply for week 4
	...
	describe('3 Accounts issue 10K dUSD each in week 1', async () => {
		beforeEach(async () => {
			await dpassive.issueSynths(tenK, { from: account1 });
			await dpassive.issueSynths(tenK, { from: account2 });
			await dpassive.issueSynths(tenK, { from: account3 });
		});

		it('Acc1 issues and burns multiple times and should have accounts 1,2,3 rewards 50%,25%,25%', async () => {
			// Acc 1 Issues 20K dUSD
			await dpassive.issueSynths(tenK, { from: account1 });
			...
			// Acc1 Burns all
			await dpassive.burnSynths(twentyK, { from: account1 });
			// Acc 1 Issues 10K dUSD
			await dpassive.issueSynths(tenK, { from: account1 });
			// Acc 1 Issues 10K dUSD again
			await dpassive.issueSynths(tenK, { from: account1 });
	...
	describe('Collateralisation Ratio Penalties', async () => {
		...
		it('should apply no penalty when users claim rewards above the penalty threshold ratio of 1%', async () => {
			// Decrease DPS collateral price by .9%
			const currentRate = await exchangeRates.rateForCurrency(DPS);
			const newRate = currentRate.sub(multiplyDecimal(currentRate, toUnit('0.009')));

			const timestamp = await currentTime();
			await exchangeRates.updateRates([DPS], [newRate], timestamp, {
				from: oracle,
			});
			...
			const dpsRewards = await feePool.feesAvailable(account1);
			assert.bnClose(dpsRewards[1], third(periodOneMintableSupplyMinusMinterReward));
		...
		it('should block user from claiming fees and rewards when users claim rewards >10% threshold collateralisation ratio', async () => {
			// But if the price of DPS decreases a lot...
			const newRate = (await exchangeRates.rateForCurrency(DPS)).sub(toUnit('0.09'));
			const timestamp = await currentTime();
			await exchangeRates.updateRates([DPS], [newRate], timestamp, {
				from: oracle,
			});
	...
	describe('When user is the last to call claimFees()', () => {
		beforeEach(async () => {
			...
			await dpassive.issueSynths(oneThousand, { from: account2 });
			await dpassive.issueSynths(oneThousand, { from: account1 });

			await dpassive.exchange(dUSD, oneThousand, dAUD, { from: account2 });
			await dpassive.exchange(dUSD, oneThousand, dAUD, { from: account1 });
		...
		it('then account gets remainder of fees/rewards available after wei rounding', async () => {
			...
			const olddUSDBalance = await dUSDContract.balanceOf(account2);
			...
			const newUSDBalance = await dUSDContract.balanceOf(account2);
			...
			// Simulate rounding on dUSD leaving fraction less for the last claimer.
			// No need to simulate for DPS as the 1.44M DPS has a 1 wei rounding already
			...
			// is entitled to 721,053.846153846153846154 DPS
			...
			assert.eventEqual(transaction, 'FeesClaimed', {
				dUSDAmount: feesAvailableUSDAcc1[0].sub(toUnit('0.000000000000000001')),
				dpsRewards: feesAvailableUSDAcc1[1].sub(toUnit('0.000000000000000001')),
			});
		});
	});
});
