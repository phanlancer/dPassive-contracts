...
contract('RewardsDistribution', async accounts => {
	...
	let rewardsDistribution, dpassive, feePool, mockRewardsRecipient;
	...
	before(async () => {
		({
			...
			accounts,
			contracts: ['RewardsDistribution', 'DPassive', 'FeePool', 'Issuer'],
		}));
	...
	it('should set constructor params on deployment', async () => {
		...
		assert.equal(await instance.dpassiveProxy(), account3);
	...
	describe('when the authority is distributing rewards', async () => {
		beforeEach(async () => {
			...
			// Set the DPS Token Transfer Address
			await rewardsDistribution.setDPassiveProxy(dpassive.address, {
				from: owner,
			});
		...
		it('should send the correct amount of tokens to the listed addresses', async () => {
			...
			// Transfer DPS to the RewardsDistribution contract address
			await dpassive.methods['transfer(address,uint256)'](
			...
			const balanceOfRewardsContract = await dpassive.balanceOf(rewardsDistribution.address);
			...
			const balanceOfAccount1 = await dpassive.balanceOf(account1);
			assert.bnEqual(balanceOfAccount1, toUnit('5000'));

			// Check Account 2 balance
			const balanceOfAccount2 = await dpassive.balanceOf(account2);
			assert.bnEqual(balanceOfAccount2, toUnit('10000'));

			// Check Reward Escrow balance
			const balanceOfRewardEscrow = await dpassive.balanceOf(rewardEscrowAddress);
		...
		it('should call the notifyRewardAmount on mockRewardsRecipient', async () => {
			...
			// Transfer DPS to the RewardsDistribution contract address
			await dpassive.transfer(rewardsDistribution.address, totalToDistribute, { from: owner });
			...
			// Check Account 2 balance
			const balanceOfMockRewardsRecipient = await dpassive.balanceOf(mockRewardsRecipient.address);
