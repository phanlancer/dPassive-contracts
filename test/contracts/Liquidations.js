...
contract('Liquidations', accounts => {
	...
	before(async () => {
		({
			...
			contracts: [
				'AddressResolver',
				'ExchangeRates',
				'Exchanger', // required for DPassive to check if exchanger().hasWaitingPeriodOrSettlementOwing
				'FeePool',
	...
	const updateDPSPrice = async rate => {
		timestamp = await currentTime();
		await exchangeRates.updateRates([DPS], [rate].map(toUnit), timestamp, {
			from: oracle,
		});
	...
	describe('with issuanceRatio of 0.125', () => {
		...
		describe('system staleness checks', () => {
			...
			describe('when the system is suspended', () => {
				...
				it('when liquidateDelinquentAccount() is invoked, it reverts with operation prohibited', async () => {
					await assert.revert(
						dpassive.liquidateDelinquentAccount(alice, toUnit('10'), { from: owner }),
		...
		describe('when anyone calls liquidateDelinquentAccount on alice', () => {
			...
			describe('then do liquidation checks', () => {
				beforeEach(async () => {
					...
					await Promise.all([dpassive.rebuildCache(), issuer.rebuildCache()]);
				});
			...
			describe('when Alice is undercollateralized', () => {
				beforeEach(async () => {
					...
					// Drop DPS value to $1 (Collateral worth $800 after)
				...
				describe('when Bob flags Alice for liquidation', () => {
					...
					describe('when deadline has passed and Alice issuance ratio is fixed as DPS price increases', () => {
						beforeEach(async () => {
							...
							await updateDPSPrice(toUnit('6'));

							const liquidationRatio = await liquidations.liquidationRatio();

							const ratio = await dpassive.collateralisationRatio(alice);
					...
					describe('when the price of DPS increases', () => {
					...
					describe('given the liquidation deadline has passed ', () => {
						...
						describe('when Alice c-ratio is above the liquidation Ratio and Bob liquidates alice', () => {
							...
							it('then Alice still has 800 DPS', async () => {
								assert.bnEqual(await dpassive.collateral(alice), toUnit('800'));
							});
						});
						...
						describe('when Alice burnSynthsToTarget to fix her c-ratio ', () => {
							...
							beforeEach(async () => {
								await updateDPSPrice('1');
								burnTransaction = await dpassive.burnSynthsToTarget({ from: alice });
							});
						...
						describe('when Alice does not fix her c-ratio ', () => {
							...
							describe('when Bob liquidates alice for 100 dUSD to get 110 DPS', () => {
								...
								beforeEach(async () => {
									// send bob some DPS
									await dpassive.transfer(bob, toUnit('1000'), {
										from: owner,
									});
								...
								it('then Alice has less DPS + penalty', async () => {
									...
									const difference = aliceDPSBefore.sub(aliceDPSAfter);
									assert.bnEqual(difference, DPS110);
								});
								it('then Bob has extra 100 DPS + the 10 DPS penalty (110)', async () => {
									const dpsBalance = await dpassive.balanceOf(bob);
									assert.bnEqual(dpsBalance, bobDPSBefore.add(DPS110));
								});
								it('then Alice DPS balance is 690', async () => {
									const aliceDPSAfter = await dpassive.collateral(alice);
									assert.bnEqual(aliceDPSAfter, toUnit('690'));
								});
								it('then Alice issuance ratio is updated in feePoolState', async () => {
									...
									const issuanceState = await dpassiveState.issuanceData(alice);
								...
								describe('given carol has obtained dUSD to liquidate alice', () => {
									...
									describe('when carol liquidates Alice with 10 x 5 dUSD', () => {
										...
										it('then Alice has less DPS + penalty', async () => {
											const aliceDPSAfter = await dpassive.collateral(alice);
											const difference = aliceDPSBefore.sub(aliceDPSAfter);
											assert.bnEqual(difference, DPS55);
										});
										it('then Carol has extra 50 DPS + the 5 DPS penalty (55)', async () => {
											const dpsBalance = await dpassive.balanceOf(carol);
											assert.bnEqual(dpsBalance, carolDPSBefore.add(DPS55));
										});
										it('then Alice DPS balance is 635', async () => {
											const aliceDPSAfter = await dpassive.collateral(alice);
											assert.bnEqual(aliceDPSAfter, toUnit('635'));
										});
										it('then Alice issuance ratio is updated in feePoolState', async () => {
											const accountsDebtEntry = await feePoolState.getAccountsDebtEntry(alice, 0);
											const issuanceState = await dpassiveState.issuanceData(alice);
									...
									describe('when carol liquidates Alice with 50 dUSD', () => {
										...
										describe('when Bob liqudates Alice with 1000 dUSD', () => {
											...
											it('then Alice issuanceRatio is now at the target issuanceRatio', async () => {
												const aliceCRatioAfter = await dpassive.collateralisationRatio(alice);
							...
							describe('given Alice has $600 Debt, $800 worth of DPS Collateral and c-ratio at 133.33%', () => {
								describe('when bob calls liquidate on Alice in multiple calls until fixing the ratio', () => {
									...
									it('then Bob can liquidate Alice multiple times until fixing the c-ratio', async () => {
										...
										while (iterations > 0) {
											await dpassive.liquidateDelinquentAccount(alice, liquidateAmount, {
												from: bob,
											});
		...
		describe('Given Alice has DPS and never issued any debt', () => {
			beforeEach(async () => {
				await dpassive.transfer(alice, toUnit('100'), { from: owner });
			});
			...
			describe('when Bob flags and tries to liquidate David', () => {
				...
				describe('when Bob liquidates all of davids collateral', async () => {
					...
					it('then David should have 0 collateral', async () => {
						assert.bnEqual(await dpassive.collateral(david), toUnit('0'));
					});
					it('then David should have a collateral ratio of 0', async () => {
						const davidCRatioAfter = await dpassive.collateralisationRatio(david);
