...
/**
 * Create a mock ExternStateToken - useful to mock DPassive or a synth
 */
...
const setupContract = async ({
	...
	const defaultArgs = {
		...
		DPassiveState: [owner, ZERO_ADDRESS],
		...
		BaseDPassive: [
			tryGetAddressOf('ProxyERC20BaseDPassive'),
			tryGetAddressOf('TokenStateBaseDPassive'),
			owner,
			SUPPLY_100M,
			tryGetAddressOf('AddressResolver'),
		],
		DPassive: [
			tryGetAddressOf('ProxyERC20DPassive'),
			tryGetAddressOf('TokenStateDPassive'),
			owner,
			SUPPLY_100M,
			tryGetAddressOf('AddressResolver'),
		],
	...
	const postDeployTasks = {
		async Issuer() {
			await Promise.all(
				[].concat(
					// DPassive State is where the issuance data lives so it needs to be connected to Issuer
		...
		async DPassive() {
			// first give all DPS supply to the owner (using the hack that the deployerAccount was setup as the associatedContract via
			// the constructor args)
			await cache['TokenStateDPassive'].setBalanceOf(owner, SUPPLY_100M, {
				from: deployerAccount,
			});

			// then configure everything else (including setting the associated contract of TokenState back to the DPassive contract)
			await Promise.all(
				[
					(cache['TokenStateDPassive'].setAssociatedContract(instance.address, { from: owner }),
					cache['ProxyDPassive'].setTarget(instance.address, { from: owner }),
					cache['ProxyERC20DPassive'].setTarget(instance.address, { from: owner }),
					instance.setProxy(cache['ProxyERC20DPassive'].address, {
						from: owner,
					})),
				]
					.concat(
						...
						tryInvocationIfNotMocked({
							name: 'SupplySchedule',
							fncName: 'setDPassiveProxy',
							args: [cache['ProxyERC20DPassive'].address],
						}) || []
					)
					.concat(
					...
					.concat(
						tryInvocationIfNotMocked({
							name: 'RewardsDistribution',
							fncName: 'setDPassiveProxy',
							args: [cache['ProxyERC20DPassive'].address], // will fail if no Proxy instantiated for DPassive
						}) || []
					)
			);
		},
		async BaseDPassive() {
			// first give all DPS supply to the owner (using the hack that the deployerAccount was setup as the associatedContract via
			// the constructor args)
			await cache['TokenStateBaseDPassive'].setBalanceOf(owner, SUPPLY_100M, {
				from: deployerAccount,
			});
			...
			await Promise.all(
				[
					(cache['TokenStateBaseDPassive'].setAssociatedContract(instance.address, {
						from: owner,
					}),
					cache['ProxyBaseDPassive'].setTarget(instance.address, { from: owner }),
					cache['ProxyERC20BaseDPassive'].setTarget(instance.address, { from: owner }),
					instance.setProxy(cache['ProxyERC20BaseDPassive'].address, {
						from: owner,
					})),
				]
					.concat(
					...
					.concat(
						tryInvocationIfNotMocked({
							name: 'RewardsDistribution',
							fncName: 'setDPassiveProxy',
							args: [cache['ProxyERC20BaseDPassive'].address], // will fail if no Proxy instantiated for BaseDPassive
						}) || []
					)
			);
		},
...
const setupAllContracts = async ({
	...
	const baseContracts = [
		...
		{ contract: 'ProxyERC20', forContract: 'DPassive' },
		{ contract: 'ProxyERC20', forContract: 'BaseDPassive' },
		{ contract: 'ProxyERC20', forContract: 'Synth' }, // for generic synth
		{ contract: 'Proxy', forContract: 'DPassive' },
		{ contract: 'Proxy', forContract: 'BaseDPassive' },
		{ contract: 'Proxy', forContract: 'FeePool' },
		{ contract: 'TokenState', forContract: 'DPassive' },
		{ contract: 'TokenState', forContract: 'BaseDPassive' },
		{ contract: 'TokenState', forContract: 'Synth' }, // for generic synth
		{ contract: 'RewardEscrow' },
		{ contract: 'BaseRewardEscrowV2', deps: ['AddressResolver'], mocks: ['DPassive', 'FeePool'] },
		{
			contract: 'RewardEscrowV2',
			deps: ['AddressResolver', 'SystemStatus'],
			mocks: ['DPassive', 'FeePool', 'RewardEscrow', 'Issuer'],
		},
		...
		{ contract: 'Liquidations', deps: ['EternalStorage', 'FlexibleStorage'] },
		{
			...
			mocks: ['DPassive', 'FeePool', 'RewardEscrow', 'RewardEscrowV2', 'ProxyFeePool'],
		},
		...
		{ contract: 'WETH' },
		{ contract: 'EtherWrapper', mocks: [], deps: ['AddressResolver', 'WETH'] },
		{
		...
		},
		{
			...
			mocks: ['DPassive', 'FeePool', 'DelegateApprovals', 'VirtualSynthMastercopy'],
			deps: [
				'AddressResolver',
				'TradingRewards',
				'SystemStatus',
		...
		{ contract: 'TradingRewards', deps: ['AddressResolver', 'DPassive'] },
		{
			contract: 'FeePool',
			mocks: [
				'DPassive',
	...
	for (const { contract, source, mocks = [], forContract } of contractsToFetch) {
		...
		if (contract === 'BaseDPassive') {
			contractRegistered = 'DPassive';
		}
	...
	await Promise.all(
		Object.entries(returnObj)
			...
			.map(([contract, instance]) => {
				console.log('instance.rebuildCache() for ', instance);
	...
	// if deploying a real DPassive, then we add the synths
