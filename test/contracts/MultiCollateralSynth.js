...
contract('MultiCollateralSynth @gas-skip', accounts => {
	...
	describe('when a MultiCollateral synth is added and connected to DPassive', () => {
		beforeEach(async () => {
		...
		describe('when multiCollateral is set to the owner', () => {
			beforeEach(async () => {
				...
				await exchangeRates.updateRates([toBytes32('dXYZ')], [toUnit(5)], timestamp, {
					from: oracle,
				});
			});
			describe('when multiCollateral tries to issue', () => {
				it('then it can issue new synths', async () => {
					...
					await ceth.open(issueAmount, toBytes32('dXYZ'), { value: toUnit(2), from: account1 });
			...
			describe('when multiCollateral tries to burn', () => {
				it('then it can burn synths', async () => {
					...
					const tx = await ceth.open(amount, toBytes32('dXYZ'), {
						value: toUnit(2),
						from: account1,
					});
			...
			describe('when dpassive set to account1', () => {
				beforeEach(async () => {
