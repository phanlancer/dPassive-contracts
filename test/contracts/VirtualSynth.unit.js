...
contract('VirtualSynth (unit tests)', async accounts => {
	...
	describe('with common setup', () => {
		...
		describe('initialize', () => {
			const amount = '1001';
			behaviors.whenInstantiated({ amount, user: owner, synth: 'dBTC' }, () => {
				it('is initialized', async () => {
					assert.isTrue(await this.instance.initialized());
				});

				it('and each initialize arg is set correctly', async () => {
					assert.equal(trimUtf8EscapeChars(await this.instance.name()), 'Virtual Synth dBTC');
					assert.equal(trimUtf8EscapeChars(await this.instance.symbol()), 'vdBTC');
		...
		describe('secsLeftInWaitingPeriod()', () => {
			behaviors.whenInstantiated({ amount: '1000', user: owner, synth: 'dBTC' }, () => {
		...
		describe('readyToSettle()', () => {
			behaviors.whenInstantiated({ amount: '999', user: owner, synth: 'dBTC' }, () => {
				behaviors.whenMockedWithMaxSecsLeft({ maxSecsLeft: 100 }, () => {
					...
					describe('settle()', () => {
						behaviors.whenSettlementCalled({ user: owner }, () => {
							it('then Exchanger.settle() is invoked with the correct params', async () => {
								...
								assert.equal(this.mocks.Exchanger.smocked.settle.calls[0][1], toBytes32('dBTC'));
							});
