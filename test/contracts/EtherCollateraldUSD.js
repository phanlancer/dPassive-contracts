...
contract('EtherCollateraldUSD', async accounts => {
	const MINUTE = 60;
	const DAY = 86400;
	const WEEK = 604800;
	const MONTH = 2629743;
	...
	const issuedUSDToAccount = async (issueAmount, receiver) => {
		...
		await dUSDSynth.transfer(receiver, issueAmount, {
			from: owner,
		});
	};
	...
	before(async () => {
		// Mock DPS, dUSD
		[{ token: dUSDSynth }] = await Promise.all([
			mockToken({ accounts, synth: 'dUSD', name: 'Synthetic USD', symbol: 'dUSD' }),
		]);

		({
			EtherCollateraldUSD: etherCollateral,
			FeePool: feePool,
	...
	describe('when accessing the external views then', async () => {
		it('loanAmountFromCollateral should return 66.66 dUSD when opening a loan with 1 ETH @ 100', async () => {
		...
		it('loanAmountFromCollateral should return 10MM dUSD when opening a loan with 150000 ETH @ 100', async () => {
		...
		it('collateralAmountForLoan should return 1 ETH @ 100 required to open 66.66 dUSD', async () => {
		...
		it('collateralAmountForLoan should return 150000 ETH @ 100 required to open 10M dUSD', async () => {
	...
	describe('when opening a Loan', async () => {
		...
		describe('then revert when ', async () => {
			...
			it('attempting to issue more than the cap (issueLimit)', async () => {
				// limit dUSD supply cap to 100
				await etherCollateral.setIssueLimit(toUnit('100'), { from: owner });
				// 3 ETH will issue up to 200 dUSD
				await assert.revert(
			...
			it('attempting to issue more near the supply cap', async () => {
				// reduce the supply cap to 100 dUSD
		...
		describe('then create loan and', async () => {
			...
			const expecteddUSDLoanAmount = calculateLoanAmount(tenETH);
			const expectedMintingFee = calculateMintingFee(expecteddUSDLoanAmount);
			const expectedTotalLoan = expecteddUSDLoanAmount.add(expectedMintingFee);
			let openLoanAmount;
			...
			it('increase the totalIssuedSynths', async () => {
				assert.bnEqual(await etherCollateral.totalIssuedSynths(), expecteddUSDLoanAmount);
			});
			it('emit a LoanCreated event', async () => {
				assert.eventEqual(openLoanTransaction, 'LoanCreated', {
					account: address1,
					loanID: 1,
					amount: expecteddUSDLoanAmount,
				});
			});
			...
			it('add the loan issue amount minus minting fees to creators balance', async () => {
				const issuanceFee = calculateMintingFee(expecteddUSDLoanAmount);
				const dUSDBalance = await dUSDSynth.balanceOf(address1);
				assert.bnEqual(dUSDBalance, expecteddUSDLoanAmount.sub(issuanceFee));
			});
			...
			describe('then close the only loan open in the system', () => {
				...
				beforeEach(async () => {
					...
					await issuedUSDToAccount(toUnit('10000'), address1);
					// Go into the future
			...
			describe('when opening a second loan against address1', async () => {
				...
				const expecteddUSDLoanAmount = calculateLoanAmount(fiveThousandETH);
				const expectedMintingFee = calculateMintingFee(expecteddUSDLoanAmount);
				const expecteddUSDTotalLoan = expecteddUSDLoanAmount.add(expectedMintingFee);
				...
				it('then increase the totalIssuedSynths', async () => {
					assert.bnEqual(
						...
						totalIssuedSynthsBefore.add(expecteddUSDTotalLoan)
					);
				});
				...
				describe('when opening loan3 against address2', async () => {
					...
					const expecteddUSDLoanAmount = calculateLoanAmount(threeNintyETH);
					...
					it('then increase the totalIssuedSynths', async () => {
						assert.bnEqual(
							await etherCollateral.totalIssuedSynths(),
							totalSupplyBefore.add(expecteddUSDLoanAmount)
						);
					});
					...
					describe('when closing the first loan of address1', async () => {
						...
						describe('when closing the second loan of address1', async () => {
							...
							describe('when closing loanID 3 from address2', async () => {
								// let calculatedFeedUSD;
								let closeLoanTransaction;

								beforeEach(async () => {
									// calculatedFeedUSD = await calculateLoanInterestFees(address2, loan3ID);
									// console.log('calculatedFeedUSD', calculatedFeedUSD.toString());

									// User will have had to made some positive trades to cover the interest
									await issuedUSDToAccount(toUnit('1000'), address2);
		...
		xdescribe('when creating multiple loans', () => {
			...
			beforeEach(async () => {
				// Issue dUSD to pay back interest
				await issuedUSDToAccount(toUnit('1000'), address1);
				await issuedUSDToAccount(toUnit('1000'), address2);
				await issuedUSDToAccount(toUnit('1000'), address3);
			});
			...
			it('then opening & closing from 10 different accounts', async () => {
				...
				for (let i = 0; i < first10Accounts.length; i++) {
					...
					// Issue dUSD to pay back interest
					await issuedUSDToAccount(toUnit('1000'), first10Accounts[i]);
				}
		...
		describe('when closing a Loan', async () => {
			...
			describe('check conditions', async () => {
				...
				it('when dUSD balance is less than loanAmount, then it reverts', async () => {
					// "Burn" some of accounts dUSD by sending to the owner
					await dUSDSynth.transfer(owner, toUnit('4'), { from: address1 });
				...
				describe('potential blocking conditions', () => {
					['System', 'Issuance'].forEach(section => {
						describe(`when ${section} is suspended`, () => {
							beforeEach(async () => {
								await issuedUSDToAccount(toUnit('100'), address1);
								await setStatus({ owner, systemStatus, section, suspend: true });
							});
					...
					describe('when rates have gone stale', () => {
						beforeEach(async () => {
							...
							issuedUSDToAccount(toUnit('100'), address1);
						});
						it('then calling closeLoan() reverts', async () => {
							await assert.revert(
								etherCollateral.closeLoan(loanID, {
						...
						describe('when dUSD gets a rate', () => {
							beforeEach(async () => {
								await updateRatesWithDefaults();
							});
							it('then calling closeLoan() succeeds', async () => {
			...
			describe('then it closes the loan and', async () => {
				...
				beforeEach(async () => {
					...
					feePoolBalanceBefore = await dUSDSynth.balanceOf(FEE_ADDRESS);
					// interestRatePerSec = await etherCollateral.interestPerSecond();
					...
					// Cacluate the fees
					expectedFeedUSD = await calculateLoanInterestFees(address1, openLoanID);
				});
	...
	describe('when loanLiquidation is opened', async () => {
		...
		const expecteddUSDLoanAmount = calculateLoanAmount(oneETH);
		...
		beforeEach(async () => {
			...
			// Chad opens dUSD loan to liquidate Alice
		...
		it('when bob attempts to liquidate alices loan and he has no dUSD then it reverts', async () => {
			await assert.revert(
		...
		xit('then alice has a dUSD loan balance', async () => {
			assert.bnEqual(await dUSDSynth.balanceOf(alice), expecteddUSDLoanAmount);
		});
		...
		describe('when loanLiquidation is open', () => {
			...
			describe('when chad has some dUSD and alice has her dUSD still', () => {
				beforeEach(async () => {
					// Chad has already opened a dUSD loan
				});
				describe('and chad liquidates alices dUSD loan for her ETH', async () => {
					...
					it('then alice dUSD balance is still intact', async () => {
						assert.ok(await dUSDSynth.balanceOf(alice), expecteddUSDLoanAmount);
					});
					it('then chads dUSD balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await dUSDSynth.balanceOf(chad), 0);
					});
			...
			describe('when bob has some dUSD', () => {
				beforeEach(async () => {
					await dUSDSynth.transfer(bob, await dUSDSynth.balanceOf(alice), { from: alice });
					await issuedUSDToAccount(toUnit('100'), bob);
				});
				describe('and bob liquidates alices dUSD loan for her ETH', async () => {
					let liquidateLoanTransaction;
					beforeEach(async () => {
						liquidateLoanTransaction = await etherCollateral.liquidateUnclosedLoan(alice, loanID, {
							from: bob,
					...
					it('then alice dUSD balance is 0 (because she transfered it to bob)', async () => {
						assert.ok(await dUSDSynth.balanceOf(alice), 0);
					});
					it('then bobs dUSD balance is 0 as it was burnt to repay the loan', async () => {
						assert.ok(await dUSDSynth.balanceOf(bob), 0);
					});
					it('then emits a LoanLiquidated event', async () => {
						assert.eventsEqual(
							liquidateLoanTransaction,
	...
	describe('when a loan is partially repaid', async () => {
		...
		it('should revert if the sender does not have enough dUSD to repay the amount requested', async () => {
			dUSDSynth.transfer(bob, await dUSDSynth.balanceOf(alice), { from: alice });
			await assert.revert(
				etherCollateral.repayLoan(alice, loanID, openLoanAmount, { from: alice }),
				'Not enough dUSD balance'
			);
		});
