...
contract('ExchangeState', accounts => {
	...
	const [dUSD, dBTC, dAUD] = ['dUSD', 'dBTC', 'dAUD'].map(toBytes32);
	...
	describe('adding, removing, selecting and length of entries', () => {
		it('the length is 0 by default', async () => {
			const length = await exchangeState.getLengthOfEntries(account1, dBTC);
			assert.equal(length, '0');
		});
		it('only the associated contract can invoke appendExchangeEntry()', async () => {
			await onlyGivenAddressCanInvoke({
				fnc: exchangeState.appendExchangeEntry,
				args: [account1, dUSD, toUnit('1'), dBTC, toUnit('1'), toUnit('0.01'), '0', '0', '0'],
		...
		describe('when an entry is added to dBTC for the first user', () => {
			...
			it('then the length is 1 for that user and synth', async () => {
				assert.equal((await exchangeState.getLengthOfEntries(account1, dBTC)).toString(), '1');
			});
			it('and the length is 0 for other conditions', async () => {
				assert.equal((await exchangeState.getLengthOfEntries(account1, dUSD)).toString(), '0');
				assert.equal((await exchangeState.getLengthOfEntries(account2, dBTC)).toString(), '0');
			});
			describe('when the entry is fetch by index 0', () => {
				let result;
				beforeEach(async () => {
					result = await exchangeState.getEntryAt(account1, dBTC, '0');
				});
			...
			describe('when another entry is added for the same user and synth', () => {
				...
				it('then the length is 2 for that user and synth', async () => {
					assert.equal((await exchangeState.getLengthOfEntries(account1, dBTC)).toString(), '2');
				});
				...
				describe('when the entry is fetch by index 1', () => {
					let result;
					beforeEach(async () => {
						result = await exchangeState.getEntryAt(account1, dBTC, '1');
					});
				...
				describe('when all entries are removed for that user and synth', () => {
					beforeEach(async () => {
						await exchangeState.removeEntries(account1, dBTC, {
							from: simulatedAssociatedContract,
						});
					});
					it('then the length is 0 for that user and synth', async () => {
						assert.equal((await exchangeState.getLengthOfEntries(account1, dBTC)).toString(), '0');
					});
				});
			});
		});
	...
	describe('getMaxTimestamp()', () => {
		it('returns 0 when no entries', async () => {
			assert.equal((await exchangeState.getMaxTimestamp(account1, dAUD)).toString(), '0');
		});
		describe('when there is a single entry with timestamp 100', () => {
			beforeEach(async () => {
				await addExchangeEntry({ user: account1, dest: dAUD, timestamp: '100' });
			});
			it('then getMaxTimestamp() must return 100', async () => {
				assert.equal((await exchangeState.getMaxTimestamp(account1, dAUD)).toString(), '100');
			});
			describe('when there is another entry with a different src and with timestamp 101', () => {
				beforeEach(async () => {
					await addExchangeEntry({ user: account1, src: dBTC, dest: dAUD, timestamp: '101' });
				});
				it('then getMaxTimestamp() must return 101', async () => {
					assert.equal((await exchangeState.getMaxTimestamp(account1, dAUD)).toString(), '101');
				});
				describe('when there is a another entry with timestamp 50', () => {
					beforeEach(async () => {
						await addExchangeEntry({ user: account1, dest: dAUD, timestamp: '50' });
					});
					it('then getMaxTimestamp() must return 101', async () => {
						assert.equal((await exchangeState.getMaxTimestamp(account1, dAUD)).toString(), '101');
					});
					describe('when there are unrelated entries at higher timestamps than 101', () => {
						beforeEach(async () => {
							await addExchangeEntry({ user: account1, dest: dBTC, timestamp: '500' });
							await addExchangeEntry({ user: account2, dest: dAUD, timestamp: '600' });
						});
