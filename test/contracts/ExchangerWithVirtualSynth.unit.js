						...
						behaviors.whenMockedToAllowChecks(() => {
							it('it reverts when called by regular accounts', async () => {
								await onlyGivenAddressCanInvoke({
									...
									args,
									accounts: accounts.filter(a => a !== this.mocks.DPassive.address),
									reason: 'Exchanger: Only dpassive or a synth contract can perform this action',
									// address: this.mocks.DPassive.address (doesnt work as this reverts due to lack of mocking setup)
								});
							});
						});
							...
							it('it reverts when either rate is invalid', async () => {
								await assert.revert(
									this.instance.exchangeWithVirtual(
										...args.concat({ from: this.mocks.DPassive.address })
									),
