...
contract('DPassiveEscrow', async accounts => {
	const DAY = 86400;
	const WEEK = 604800;
	const YEAR = 31556926;
	...
	describe('Constructor & Settings ', async () => {
		it('should set dpassive on contructor', async () => {
			const dpassiveAddress = await escrow.dpassive();
		...
		it('should allow owner to set dpassive', async () => {
			await escrow.setDPassive(ZERO_ADDRESS, { from: owner });
			const dpassiveAddress = await escrow.dpassive();
	...
	describe('Only During Setup', async () => {
		it('should allow owner to purgeAccount', async () => {
			...
			await dpassive.transfer(escrow.address, toUnit('1000'), {
				from: owner,
			});
		...
		it('should allow owner to call addVestingSchedule', async () => {
			// Transfer of DPS to the escrow must occur before creating an entry
			await dpassive.transfer(escrow.address, toUnit('200'), {
				from: owner,
			});
	...
	describe('Functions', async () => {
		describe('Vesting Schedule Writes', async () => {
			it('should not create a vesting entry with a zero amount', async () => {
				// Transfer of DPS to the escrow must occur before creating an entry
				await dpassive.transfer(escrow.address, toUnit('1'), {
					from: owner,
				});
		...
		describe('Vesting Schedule Reads ', async () => {
			beforeEach(async () => {
				...
				await dpassive.transfer(escrow.address, toUnit('6000'), {
					from: owner,
				});
			...
			it('should append a vesting entry and increase the contracts balance', async () => {
				const balanceOfRewardEscrow = await dpassive.balanceOf(escrow.address);
		...
		describe('Vesting', async () => {
			...
			it('should vest and transfer dps from contract to the user', async () => {
				...
				// Check escrow does not have any DPS
				assert.bnEqual(await dpassive.balanceOf(escrow.address), toUnit('0'));
			});
