...
contract('BinaryOptionMarketManager @gas-skip', accounts => {
	...
	let manager, factory, systemStatus, exchangeRates, addressResolver, dUSDSynth, oracle;
	...
	before(async () => {
		...
		await Promise.all([
			dUSDSynth.issue(initialCreator, dUSDQty),
			dUSDSynth.approve(manager.address, dUSDQty, { from: initialCreator }),
			dUSDSynth.issue(bidder, dUSDQty),
			dUSDSynth.approve(manager.address, dUSDQty, { from: bidder }),
		]);
	});
	...
	describe('Market creation', () => {
		...
		it('Cannot create a market without sufficient capital to cover the initial bids.', async () => {
			...
			await dUSDSynth.issue(dummy, dUSDQty);
			...
			await dUSDSynth.approve(manager.address, dUSDQty, { from: dummy });
	...
	describe('Market tracking', () => {
		it('Multiple markets can exist simultaneously, and debt is tracked properly across them.', async () => {
			...
			await Promise.all(
				markets.map(market => dUSDSynth.approve(market.address, dUSDQty, { from: bidder }))
			);
			...
			await fastForward(expiryDuration + 1000);
			await exchangeRates.updateRates([dAUDKey], [toUnit(2)], await currentTime(), {
				from: oracle,
			});
	...
	describe('Deposit management', () => {
		...
		it('Bidding affects total deposits properly.', async () => {
			...
			await dUSDSynth.issue(bidder, dUSDQty);
			await dUSDSynth.approve(market.address, dUSDQty, { from: bidder });
	...
	describe('Market migration', () => {
		...
		before(async () => {
			...
			await Promise.all(
				markets.map(m => dUSDSynth.approve(m.address, toUnit(1000), { from: bidder }))
			);
			await dUSDSynth.approve(newManager.address, toUnit(1000), { from: bidder });
		...
		it('Markets can be migrated to a manager with existing markets.', async () => {
			...
			await dUSDSynth.approve(newManager.address, toUnit('1000'));
			const now = await currentTime();
