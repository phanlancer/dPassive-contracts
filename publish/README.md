...
This script can `build` (compile and flatten), `deploy` and `verify` (on Etherscan) the DPassive code to a testnet or mainnet.
...
```bash
node publish nominate -n rinkeby -d publish/deployed/rinkeby -g 3 -c DPassive -c ProxydUSD -o 0x0000000000000000000000000000000000000000
node publish nominate -o 0xB64fF7a4a33Acdf48d97dab0D764afD0F6176882 -n kovan -c ProxydUSD -d publish/deployed/kovan -g 20
```
...
Will attempt to remove all given synths from the `DPassive` contract (as long as they have `totalSupply` of `0`) and update the `config.json` and `synths.json` for the deployment folder.
...
```bash
node publish remove-synths -n rinkeby -d publish/deployed/rinkeby -g 3 -s dRUB -s dETH
```
...
Will initiate the dpassive release process, publishing the dpassive `yarn` module and updating all dependent projects in GitHub and `yarn`.
...
For `dpassive-core-contracts` repo, we are using the following branch mapping:
