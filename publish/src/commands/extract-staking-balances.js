...
async function extractStakingBalances({ network = DEFAULTS.network, deploymentPath, synth }) {
	...
	const { abi: dpsABI } = getSource({ contract: 'DPassive' });
	...
	async function fetchStakedBalances() {
		const iSynth = new web3.eth.Contract(dpsABI, iSynthAddress);
		const stakingContract = new web3.eth.Contract(dpsABI, stakingAddress);
