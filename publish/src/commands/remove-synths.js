...
const removeSynths = async ({
		...
		try {
			await confirmAction(
				cyan(
					`${yellow(
						'⚠ WARNING'
					)}: This action will remove the following synths from the DPassive contract on ${network}:\n- ${synthsToRemove.join(
						'\n- '
					)}`
	...
	const DPassive = new web3.eth.Contract(
		deployment.sources['DPassive'].abi,
		deployment.targets['DPassive'].address
	);
	...
	for (const currencyKey of synthsToRemove) {
		...
		const currentSynthInDPS = await DPassive.methods.synths(toBytes32(currencyKey)).call();

		if (synthAddress !== currentSynthInDPS) {
			console.error(
				red(
					`Synth address in DPassive for ${currencyKey} is different from what's deployed in DPassive to the local ${DEPLOYMENT_FILENAME} of ${network} \ndeployed: ${yellow(
						currentSynthInDPS
		...
		// now check total supply (is required in DPassive.removeSynth)
		...
		// perform transaction if owner of DPassive or append to owner actions list
