...
// The block where DPassive first had SIP-37 added (when ExchangeState was added)
const fromBlockMap = {
...
const settle = async ({
	...
	try {
		...
		exchanges = await fetchAllEvents({ target: DPassive });
		saveExchangesToFile({
			network,
			fromBlock,
			exchanges,
	...
	for (const {
		...
		if (+numEntries > 0) {
			...
			if (showDebt) {
				...
				if (reclaimAmount > 0) {
					const synth = await DPassive.methods.synths(toCurrencyKey).call();
