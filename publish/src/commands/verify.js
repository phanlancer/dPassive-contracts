				...
				if (/^Proxy/.test(name)) {
					...
					const optionalProxyLink = () => {
						...
						return (
							...
							`https://contracts.dpassive.io/${network !== 'mainnet' ? network + '/' : ''}${
								name === 'DPassive' ? 'ProxyERC20' : 'Proxy' + name
							}`
						);
					};
					return (
						...
						`\nThis contract has an associated PROXY that MUST be used for all integrations - this TARGET will be REPLACED in an upcoming DPassive release!` +
						`${optionalProxyLink()}\n\n*/`
					);
				}
				return '';
