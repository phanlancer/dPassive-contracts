...
const purgeSynths = async ({
		...
		try {
			await confirmAction(
				cyan(
					`${yellow(
						'⚠ WARNING'
					)}: This action will purge the following synths from the DPassive contract on ${network}:\n- ${synthsToPurge.join(
						'\n- '
					)}`
	...
	const { address: dpassiveAddress, source } = deployment.targets['DPassive'];
	const { abi: dpassiveABI } = deployment.sources[source];
	const DPassive = new web3.eth.Contract(dpassiveABI, dpassiveAddress);
