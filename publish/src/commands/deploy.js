...
const DEFAULTS = {
	...
	contractDeploymentGasLimit: 6.9e6, // TODO split out into separate limits for different contracts, Proxys, Synths, DPassive
...
const deploy = async ({
	...
	if (!ignoreSafetyChecks) {
		...
		if (network.toLowerCase() === 'goerli' && !manageNonces) {
	...
	try {
		const oldDPassive = deployer.getExistingContract({ contract: 'DPassive' });
		currentDPassiveSupply = await oldDPassive.methods.totalSupply().call();

		// inflationSupplyToDate = total supply - 100m
		const inflationSupplyToDate = w3utils
			.toBN(currentDPassiveSupply)
		...
		if (freshDeploy) {
			currentDPassiveSupply = await getDeployParameter('INITIAL_ISSUANCE');
			currentWeekOfInflation = 0;
			currentLastMintEvent = 0;
		} else {
			console.error(
				red(
					'Cannot connect to existing DPassive contract. Please double check the deploymentPath is correct for the network allocated'
				)
			);
			process.exitCode = 1;
			return;
	...
	parameterNotice({
		...
		'DPassive totalSupply': `${Math.round(w3utils.fromWei(currentDPassiveSupply) / 1e6)}m`,
			...
			await confirmAction(
				yellow(
					`⚠⚠⚠ WARNING: This action will deploy the following contracts to ${network}:\n${Object.entries(
						...
						.join(', ')}` + `\nIt will also set proxy targets and add synths to DPassive.\n`
				) +
					gray('-'.repeat(50)) +
	...
	const dPassiveEscrow = await deployer.deployContract({
		name: 'DPassiveEscrow',
		args: [account, ZERO_ADDRESS],
	});

	const dpassiveState = await deployer.deployContract({
		name: 'DPassiveState',
		source: 'DPassiveState',
	...
	const rewardsDistribution = await deployer.deployContract({
		...
		deps: ['RewardEscrowV2', 'ProxyFeePool'],
		args: [
			account, // owner
			ZERO_ADDRESS, // authority (dpassive)
			ZERO_ADDRESS, // DPassive Proxy
	...
	// New DPassive proxy.
	const proxyERC20DPassive = await deployer.deployContract({
		name: 'ProxyERC20',
		args: [account],
	});

	const tokenStateDPassive = await deployer.deployContract({
		name: 'TokenStateDPassive',
		source: 'TokenState',
		args: [account, account],
	});

	const dpassive = await deployer.deployContract({
		name: 'DPassive',
		source: 'DPassive',
		deps: ['ProxyERC20', 'TokenStateDPassive', 'AddressResolver'],
		args: [
			addressOf(proxyERC20DPassive),
			addressOf(tokenStateDPassive),
			account,
			currentDPassiveSupply,
			addressOf(readProxyForResolver),
		],
	});

	if (dpassive && proxyERC20DPassive) {
		await runStep({
			contract: 'ProxyERC20',
			target: proxyERC20DPassive,
			read: 'target',
			expected: input => input === addressOf(dpassive),
			write: 'setTarget',
			writeArg: addressOf(dpassive),
		});
		await runStep({
			contract: 'DPassive',
			target: dpassive,
			read: 'proxy',
			expected: input => input === addressOf(proxyERC20DPassive),
			write: 'setProxy',
			writeArg: addressOf(proxyERC20DPassive),
		});
	}

	// Old DPassive proxy based off Proxy.sol: this has been deprecated.
	// To be removed after May 30, 2020:
	const proxyDPassive = await deployer.deployContract({
		name: 'ProxyDPassive',
		source: 'Proxy',
		args: [account],
	});
	if (proxyDPassive && dpassive) {
		await runStep({
			contract: 'ProxyDPassive',
			target: proxyDPassive,
			read: 'target',
			expected: input => input === addressOf(dpassive),
			write: 'setTarget',
			writeArg: addressOf(dpassive),
	...
	if (tokenStateDPassive && config['TokenStateDPassive'].deploy) {
		const initialIssuance = await getDeployParameter('INITIAL_ISSUANCE');
		await runStep({
			contract: 'TokenStateDPassive',
			target: tokenStateDPassive,
			read: 'balanceOf',
			readArg: account,
	...
	if (tokenStateDPassive && dpassive) {
		await runStep({
			contract: 'TokenStateDPassive',
			target: tokenStateDPassive,
			read: 'associatedContract',
	...
	if (dpassiveState && issuer) {
		// The DPassiveState contract has Issuer as it's associated contract (after v2.19 refactor)
		await runStep({
			contract: 'DPassiveState',
			target: dpassiveState,
			read: 'associatedContract',
	...
	if (dPassiveEscrow) {
		await deployer.deployContract({
			name: 'EscrowChecker',
			deps: ['DPassiveEscrow'],
			args: [addressOf(dPassiveEscrow)],
		});
	}

	if (rewardEscrow && dpassive) {
		await runStep({
	...
	if (supplySchedule && dpassive) {
		await runStep({
			contract: 'SupplySchedule',
			target: supplySchedule,
			read: 'dpassiveProxy',
			expected: input => input === addressOf(proxyDPassive),
			write: 'setDPassiveProxy',
			writeArg: addressOf(proxyDPassive),
		});
	}

	if (dpassive && rewardsDistribution) {
		await runStep({
	...
	// ----------------
	// Setting proxyERC20 DPassive for dPassiveEscrow
	// ----------------

	// Skip setting unless redeploying either of these,
	if (config['DPassive'].deploy || config['DPassiveEscrow'].deploy) {
		// Note: currently on mainnet DPassiveEscrow.methods.dpassive() does NOT exist
		// it is "havven" and the ABI we have here is not sufficient
		if (network === 'mainnet') {
			await runStep({
				contract: 'DPassiveEscrow',
				target: dPassiveEscrow,
				read: 'havven',
				expected: input => input === addressOf(proxyERC20DPassive),
				write: 'setHavven',
				writeArg: addressOf(proxyERC20DPassive),
	...
	for (const { name: currencyKey, subclass, asset } of synths) {
		...
		const synth = await deployer.deployContract({
			...
			deps: [`TokenState${currencyKey}`, `Proxy${currencyKey}`, 'DPassive', 'FeePool'],
			args: [
	...
	if (systemSettings) {
		...
		if (synthsRatesToUpdate.length) {
			...
			await runStep({
				gasLimit: Math.max(methodCallGasLimit, 150e3 * synthsRatesToUpdate.length), // higher gas required, 150k per synth is sufficient
	...
	const refreshSnapshotIfPossible = async (wasInvalid, isInvalid, force = false) => {
		...
		if (force || validityChanged) {
			...
			await runStep({
				gasLimit: 5.0e6, // About 3.34 million gas is required to refresh the snapshot with ~40 synths
				contract: 'DebtCache',
				target: debtCache,
