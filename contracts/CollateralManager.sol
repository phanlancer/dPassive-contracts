...
contract CollateralManager is ICollateralManager, Owned, Pausable, MixinResolver {
    ...
    function getBorrowRate() external view override returns (uint borrowRate, bool anyRateIsInvalid) {
        ...
        // the total.
        uint totalDebt = dpsDebt.add(nonDpsDebt);

        // now work out the utilisation ratio, and divide through to get a per second value.
        uint utilisation = nonDpsDebt.divideDecimal(totalDebt).divideDecimal(SECONDS_IN_A_YEAR);
    ...
    function exceedsDebtLimit(uint amount, bytes32 currency)
        external
        view
        override
