...
contract VirtualToken is ERC20 {
    ...
    constructor(
        ...
        public
        ERC20(
            string(abi.encodePacked("Virtual Token ", _targetToken.name())),
            string(abi.encodePacked("v", _targetToken.symbol()))
        )
    {
        vSynth = _vSynth;
        pool = _pool;
        targetToken = _targetToken;
        _setupDecimals(IERC20Detailed(address(vSynth.synth())).decimals());
    }

    function _synthBalance() internal view returns (uint) {
        return IERC20Detailed(address(vSynth.synth())).balanceOf(address(this));
    }

    function convert(address account, uint amount) external {
        // transfer the vSynth from the creating contract to me
        IERC20Detailed(address(vSynth)).transferFrom(msg.sender, address(this), amount);
    ...
    function internalSettle() internal {
        ...
        IERC20Detailed synth = IERC20Detailed(address(vSynth.synth()));
        ...
        // now exchange all my synths (dBTC) for WBTC
...
contract SwapWithVirtualSynth {
    ICurvePool public incomingPool = ICurvePool(0xA5407eAE9Ba41422680e2e00537571bcC53efBfD); // Curve: dUSD v2 Swap
    ICurvePool public outgoingPool = ICurvePool(0x7fC77b5c7614E1533320Ea6DDc2Eb61fa00A9714); // Curve: dBTC Swap

    IDPassive public dpassive = IDPassive(0xC011a73ee8576Fb46F5E1c5751cA3B9Fe0af2a6F);

    IERC20Detailed public dUSD = IERC20Detailed(0x57Ab1ec28D129707052df4dF418D58a2D46d5f51);
    ...
    function usdcToWBTC(uint amount) external {
        ...
        // now invoke curve USDC to dUSD
        incomingPool.exchange(1, 3, amount, 0);

        // now exchange my dUSD to dBTC
        (, IVirtualSynth vSynth) = dpassive.exchangeWithVirtual("dUSD", dUSD.balanceOf(address(this)), "dBTC", bytes32(0));
