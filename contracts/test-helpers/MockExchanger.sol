...
contract MockExchanger {
    ...
    constructor(IDPassive _dpassive) public {
        dpassive = _dpassive;
    }
        ...
        )
    {
        if (_mockReclaimAmount > 0) {
            dpassive.synths(currencyKey).burn(from, _mockReclaimAmount);
        }

        if (_mockRefundAmount > 0) {
            dpassive.synths(currencyKey).issue(from, _mockRefundAmount);
        }
