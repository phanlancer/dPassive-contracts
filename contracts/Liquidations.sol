...
contract Liquidations is Owned, MixinSystemSettings, ILiquidations {
    ...
    // refactor to dpassive storage eternal storage contract once that's ready
    ...
    function liquidationDelay() external view override returns (uint) {
        return getLiquidationDelay();
    }

    function liquidationRatio() external view override returns (uint) {
        return getLiquidationRatio();
    }

    function liquidationPenalty() external view override returns (uint) {
    ...
    function getLiquidationDeadlineForAccount(address account) external view override returns (uint) {
        LiquidationEntry memory liquidation = _getLiquidationEntryForAccount(account);
        return liquidation.deadline;
    }

    function isOpenForLiquidation(address account) external view override returns (bool) {
        uint accountCollateralisationRatio = dpassive().collateralisationRatio(account);

        // Liquidation closed if collateral ratio less than or equal target issuance Ratio
        // Account with no DPS collateral will also not be open for liquidation (ratio is 0)
    ...
    function isLiquidationDeadlinePassed(address account) external view override returns (bool) {
        LiquidationEntry memory liquidation = _getLiquidationEntryForAccount(account);
     ...
     */
    function calculateAmountToFixCollateral(uint debtBalance, uint collateral) external view override returns (uint) {
    ...
    // check DPS rate is not stale
    function flagAccountForLiquidation(address account) external override rateNotInvalid("DPS") {
        ...
        uint accountsCollateralisationRatio = dpassive().collateralisationRatio(account);
    ...
    function removeAccountInLiquidation(address account) external override onlyIssuer {
        LiquidationEntry memory liquidation = _getLiquidationEntryForAccount(account);
    ...
    // Check DPS rate is not stale
    function checkAndRemoveAccountInLiquidation(address account) external override rateNotInvalid("DPS") {
        systemStatus().requireSystemActive();
