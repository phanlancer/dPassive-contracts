...
contract DPassive is BaseDPassive {
    ...
    constructor(
        ...
        uint _totalSupply,
        address _resolver
    ) public BaseDPassive(_proxy, _tokenState, _owner, _totalSupply, _resolver) {}

    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        bytes32[] memory existingAddresses = BaseDPassive.resolverAddressesRequired();
    ...
    function mint() external override issuanceActive returns (bool) {
        ...
        // Set minted DPS balance to RewardEscrow's balance
        ...
        returns (bool)
    {
        ...
        // Transfer DPS redeemed to messageSender
        // Reverts if amount to redeem is more than balanceOf account, ie due to escrowed balance
        return _transferByProxy(account, messageSender, totalRedeemed);
    }

    /* Once off function for SIP-60 to migrate DPS balances in the RewardEscrow contract
    ...
    // ========== EVENTS ==========
    event AccountLiquidated(address indexed account, uint dpsRedeemed, uint amountLiquidated, address liquidator);
    ...
    function emitAccountLiquidated(
        ...
        proxy._emit(
            abi.encode(dpsRedeemed, amountLiquidated, liquidator),
            2,
