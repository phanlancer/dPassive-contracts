...
contract SupplySchedule is Owned, ISupplySchedule {
    ...
    // The number of DPS rewarded to the caller of DPassive.mint()
    ...
    // Address of the DPassiveProxy for the onlyDPassive modifier
    address payable public dpassiveProxy;

    // Max DPS rewards for minter
    ...
    /**
     * @return The amount of DPS mintable for the inflationary supply
     */
    function mintableSupply() external view override returns (uint) {
        uint totalAmount;
        ...
        while (remainingWeeksToMint > 0) {
            ...
            if (currentWeek < SUPPLY_DECAY_START) {
                ...
                // Terminal supply is calculated on the total supply of DPassive including any new supply
                // We can compound the remaining week's supply at the fixed terminal rate
                uint totalSupply = IERC20(dpassiveProxy).totalSupply();
    ...
    /**
     * @notice Record the mint event from DPassive by incrementing the inflation
     * week counter for the number of weeks minted (probabaly always 1)
     * and store the time of the event.
     * @param supplyMinted the amount of DPS the total supply was inflated by.
     * */
    function recordMintEvent(uint supplyMinted) external override onlyDPassive returns (bool) {
    ...
    /**
     * @notice Sets the reward amount of DPS for the caller of the public
     * function DPassive.mint().
     * This incentivises anyone to mint the inflationary supply and the mintr
     * Reward will be deducted from the inflationary supply and sent to the caller.
     * @param amount the amount of DPS to reward the minter.
     * */
    ...
    /**
     * @notice Set the DPassiveProxy should it ever change.
     * SupplySchedule requires DPassive address as it has the authority
     * to record mint event.
     * */
    function setDPassiveProxy(IDPassive _dpassiveProxy) external onlyOwner {
        require(address(_dpassiveProxy) != address(0), "Address cannot be 0");
        dpassiveProxy = address(uint160(address(_dpassiveProxy)));
        emit DPassiveProxyUpdated(dpassiveProxy);
    }

    // ========== MODIFIERS ==========

    /**
     * @notice Only the DPassive contract is authorised to call this function
     * */
    modifier onlyDPassive() {
        require(
            msg.sender == address(Proxy(payable(dpassiveProxy)).target()),
            "Only the dpassive contract can perform this action"
        );
        _;
    }
    ...
    /**
     * @notice Emitted when the DPS minter reward amount is updated
     * */
    event MinterRewardUpdated(uint newRewardAmount);

    /**
     * @notice Emitted when setDPassiveProxy is called changing the DPassive Proxy address
     * */
    event DPassiveProxyUpdated(address newAddress);
}
