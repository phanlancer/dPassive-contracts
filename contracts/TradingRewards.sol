...
contract TradingRewards is ITradingRewards, ReentrancyGuard, Owned, Pausable, MixinResolver {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        addresses[1] = CONTRACT_DPASSIVE;
    }

    function dpassive() internal view virtual returns (IERC20) {
    ...
    function getAvailableRewards() external view override returns (uint) {
        return _balanceAssignedToRewards;
    }

    function getUnassignedRewards() external view override returns (uint) {
        return dpassive().balanceOf(address(this)).sub(_balanceAssignedToRewards);
    }

    function getRewardsToken() external view override returns (address) {
        return address(dpassive());
    }

    function getPeriodController() external view override returns (address) {
    ...
    function getPeriodIsClaimable(uint periodID) external view override returns (bool) {
        return _periods[periodID].isFinalized;
    }

    function getPeriodIsFinalized(uint periodID) external view override returns (bool) {
        return _periods[periodID].isFinalized;
    }

    function getPeriodRecordedFees(uint periodID) external view override returns (uint) {
        return _periods[periodID].recordedFees;
    }

    function getPeriodTotalRewards(uint periodID) external view override returns (uint) {
        return _periods[periodID].totalRewards;
    }

    function getPeriodAvailableRewards(uint periodID) external view override returns (uint) {
        return _periods[periodID].availableRewards;
    }

    function getUnaccountedFeesForAccountForPeriod(address account, uint periodID) external view override returns (uint) {
        return _periods[periodID].unaccountedFeesForAccount[account];
    }

    function getAvailableRewardsForAccountForPeriod(address account, uint periodID) external view override returns (uint) {
    ...
    function claimRewardsForPeriod(uint periodID) external override nonReentrant notPaused {
        _claimRewards(msg.sender, periodID);
    }

    function claimRewardsForPeriods(uint[] calldata periodIDs) external override nonReentrant notPaused {
        for (uint i = 0; i < periodIDs.length; i++) {
            uint periodID = periodIDs[i];
    ...
    function _claimRewards(address account, uint periodID) internal {
        ...
        dpassive().safeTransfer(account, amountToClaim);
    ...
    function recordExchangeFeeForAccount(uint usdFeeAmount, address account) external override onlyExchanger {
    ...
    function closeCurrentPeriodWithRewards(uint rewards) external override onlyPeriodController {
        uint currentBalance = dpassive().balanceOf(address(this));
    ...
    function recoverTokens(address tokenAddress, address recoverAddress) external override onlyOwner {
        _validateRecoverAddress(recoverAddress);
        require(tokenAddress != address(dpassive()), "Must use another function");
    ...
    function recoverUnassignedRewardTokens(address recoverAddress) external override onlyOwner {
        _validateRecoverAddress(recoverAddress);

        uint tokenBalance = dpassive().balanceOf(address(this));
        require(tokenBalance > 0, "No tokens to recover");

        uint unassignedBalance = tokenBalance.sub(_balanceAssignedToRewards);
        require(unassignedBalance > 0, "No tokens to recover");

        dpassive().safeTransfer(recoverAddress, unassignedBalance);

        emit UnassignedRewardTokensRecovered(recoverAddress, unassignedBalance);
    }

    function recoverAssignedRewardTokensAndDestroyPeriod(address recoverAddress, uint periodID) external override onlyOwner {
        _validateRecoverAddress(recoverAddress);
        ...
        dpassive().safeTransfer(recoverAddress, amount);
    ...
    function setPeriodController(address newPeriodController) external override onlyOwner {
