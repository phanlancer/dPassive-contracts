...
contract SystemSettings is Owned, MixinSystemSettings, ISystemSettings {
    ...
    // No more synths may be issued than the value of DPS backing them.
    ...
    function exchangeFeeRate(bytes32 currencyKey) external view override returns (uint) {
    ...
    function etherWrapperMaxETH() external view override returns (uint) {
        return getEtherWrapperMaxETH();
    }

    // SIP 112: ETH Wrappr
    // The fee for depositing ETH into the EtherWrapper.
    function etherWrapperMintFeeRate() external view override returns (uint) {
