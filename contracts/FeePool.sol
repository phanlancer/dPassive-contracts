...
contract FeePool is Owned, Proxyable, LimitedSetup, MixinSystemSettings, IFeePool {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[1] = CONTRACT_DPASSIVE;
    ...
    function feePeriodDuration() external view override returns (uint) {
        return getFeePeriodDuration();
    }

    function targetThreshold() external view override returns (uint) {
     ...
     * @param debtEntryIndex The index in the global debt ledger. dpassiveState.issuanceData(account)
     * @dev onlyIssuer to call me on dpassive.issue() & dpassive.burn() calls to store the locked DPS
    ...
    function appendAccountIssuanceRecord(
        ...
        uint debtRatio,
        uint debtEntryIndex
    ) external override onlyIssuerAndDPassiveState {
    ...
    /**
     * @notice The RewardsDistribution contract informs us how many DPS rewards are sent to RewardEscrow to be claimed.
     */
    function setRewardsToDistribute(uint amount) external override {
        ...
        // Add the amount of DPS rewards to distribute on top of any rolling unclaimed amount
    ...
    function closeCurrentFeePeriod() external override issuanceActive {
        ...
        _recentFeePeriodsStorage(0).startingDebtIndex = uint64(dpassiveState().debtLedgerLength());
        _recentFeePeriodsStorage(0).startTime = uint64(now);
    ...
    function importFeePeriod(
        ...
        require(startingDebtIndex <= dpassiveState().debtLedgerLength(), "Cannot import bad data");
     ...
     * @param dpsAmount The amount of DPS tokens.
     */
    function _recordRewardPayment(uint dpsAmount) internal returns (uint) {
        // Don't assign to the parameter
        uint remainingToAllocate = dpsAmount;
     ...
     * @param account The address to send the fees to.
     * @param dpsAmount The amount of DPS.
     */
    function _payRewards(address account, uint dpsAmount) internal notFeeAddress(account) {
        /* Escrow the tokens for 1 year. */
        uint escrowDuration = 52 weeks;

        // Record vesting entry for claiming address and amount
        // DPS already minted to rewardEscrow balance
        rewardEscrowV2().appendVestingEntry(account, dpsAmount, escrowDuration);
    }
    ...
    /**
     * @notice The total DPS rewards available in the system to be withdrawn
     */
    function totalRewardsAvailable() external view override returns (uint) {
        uint totalRewards = 0;
    ...
    function isFeesClaimable(address account) external view override returns (bool feesClaimable) {
    ...
    function _effectiveDebtRatioForPeriod(
        ...
        // This is a high precision integer.
        IDPassiveState _dpassiveState = dpassiveState();
        uint feePeriodDebtOwnership =
            _dpassiveState
                .debtLedger(closingDebtIndex)
                .divideDecimalRoundPrecise(_dpassiveState.debtLedger(debtEntryIndex))
    ...
    modifier onlyIssuerAndDPassiveState {
        bool isIssuer = msg.sender == address(issuer());
        bool isDPassiveState = msg.sender == address(dpassiveState());
        require(isIssuer || isDPassiveState, "Issuer and DPassiveState only");
        _;
    }
