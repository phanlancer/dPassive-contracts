...
contract ExchangeRates is Owned, MixinSystemSettings, IExchangeRates {
    ...
    mapping(bytes32 => AggregatorV2V3Interface) private _aggregators;
    ...
    function addAggregator(bytes32 currencyKey, address aggregatorAddress) external onlyOwner {
        ...
        if (address(_aggregators[currencyKey]) == address(0)) {
            aggregatorKeys.push(currencyKey);
        }
        _aggregators[currencyKey] = aggregator;
    ...
    function removeAggregator(bytes32 currencyKey) external onlyOwner {
        ...
        delete _aggregators[currencyKey];
    ...
    function freezeRate(bytes32 currencyKey) external virtual override {
        InversePricing storage inverse = inversePricing[currencyKey];
    ...
    function aggregators(bytes32 currencyKey) external view override returns (address) {
        return address(_aggregators[currencyKey]);
    }
    ...
    function canFreezeRate(bytes32 currencyKey) external view virtual override returns (bool) {
    ...
    function currenciesUsingAggregator(address aggregator) external view override returns (bytes32[] memory currencies) {
        ...
        for (uint i = 0; i < aggregatorKeys.length; i++) {
            ...
            if (address(_aggregators[currencyKey]) == aggregator) {
    ...
    function rateAndTimestampAtRound(bytes32 currencyKey, uint roundId)
        external
        view
        override
    ...
    function rateIsFrozen(bytes32 currencyKey) external view virtual override returns (bool) {
    ...
    function getFlagsForRates(bytes32[] memory currencyKeys) internal view returns (bool[] memory flagList) {
        ...
        if (_flags != FlagsInterface(0)) {
            address[] memory aggregators_ = new address[](currencyKeys.length);

            for (uint i = 0; i < currencyKeys.length; i++) {
                aggregators_[i] = address(_aggregators[currencyKeys[i]]);
            }

            flagList = _flags.getFlags(aggregators_);
        } else {
    ...
    function _getRateAndUpdatedTime(bytes32 currencyKey) internal view returns (RateAndUpdatedTime memory) {
        AggregatorV2V3Interface aggregator = _aggregators[currencyKey];
