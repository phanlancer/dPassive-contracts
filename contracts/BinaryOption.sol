...
contract BinaryOption is IERC20, IBinaryOption {
    ...
    mapping(address => uint) public override(IERC20, IBinaryOption) balanceOf;
    uint public override(IERC20, IBinaryOption) totalSupply;
    ...
    constructor(address initialBidder, uint initialBid) public {
        _market = BinaryOptionMarket(msg.sender);
    ...
    function market() external view override returns (IBinaryOptionMarket) {
        return _market;
    }
    ...
    function claimableBalanceOf(address account) external view override returns (uint) {
        (uint price, uint exercisableDeposits) = _market.senderPriceAndExercisableDeposits();
    ...
    function totalClaimableSupply() external view override returns (uint) {
        (, uint exercisableDeposits) = _market.senderPriceAndExercisableDeposits();
    ...
    function _transfer(
        ...
        _market.requireActiveAndUnpaused();
    ...
    modifier onlyMarket() {
        require(msg.sender == address(_market), "Only market allowed");
        _;
    }
