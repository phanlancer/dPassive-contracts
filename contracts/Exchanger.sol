...
// Used to have strongly-typed access to internal mutative functions in DPassive
interface IDPassiveInternal {
...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    function resolverAddressesRequired() public view virtual override returns (bytes32[] memory addresses) {
        ...
        newAddresses[3] = CONTRACT_DPASSIVE;
    ...
    function waitingPeriodSecs() external view override returns (uint) {
    ...
    function hasWaitingPeriodOrSettlementOwing(address account, bytes32 currencyKey) external view override returns (bool) {
    ...
    function calculateAmountAfterSettlement(
        ...
        uint amount,
        uint refunded
    ) public view override returns (uint amountAfterSettlement) {
    ...
    function isSynthRateInvalid(bytes32 currencyKey) external view override returns (bool) {
    ...
    ) external override onlyDPassiveorSynth returns (uint amountReceived) {
        uint fee;
    ...
    function exchangeWithVirtual(
        ...
        address destinationAddress,
        bytes32 trackingCode
    ) external override onlyDPassiveorSynth returns (uint amountReceived, IVirtualSynth vSynth) {
        uint fee;
    ...
    function _emitTrackingEvent(
        ...
        IDPassiveInternal(address(dpassive())).emitExchangeTracking(trackingCode, toCurrencyKey, toAmount, fee);
    }
        ...
        )
    {
        ...
        // in these currencies
        _updateDPSIssuedDebtOnExchange([sourceCurrencyKey, destinationCurrencyKey], [sourceRate, destinationRate]);

        // Let the DApps know there was a Synth exchange
        IDPassiveInternal(address(dpassive())).emitSynthExchange(
            from,
            sourceCurrencyKey,
            sourceAmountAfterSettlement,
            destinationCurrencyKey,
    ...
    function suspendSynthWithInvalidRate(bytes32 currencyKey) external override {
    ...
    function reclaim(
        ...
        IDPassiveInternal(address(dpassive())).emitExchangeReclaim(from, currencyKey, amount);
    }
    ...
    function refund(
        ...
        IDPassiveInternal(address(dpassive())).emitExchangeRebate(from, currencyKey, amount);
    }
    ...
    modifier onlyDPassiveorSynth() {
        IDPassive _dpassive = dpassive();
        require(
            msg.sender == address(_dpassive) || _dpassive.synthsByAddress(msg.sender) != bytes32(0),
            "Exchanger: Only dpassive or a synth contract can perform this action"
        );
        _;
    }
