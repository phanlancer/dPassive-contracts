...
contract FeePoolState is Owned, LimitedSetup {
     ...
     * @param debtEntryIndex The index in the global debt ledger. dpassive.dpassiveState().issuanceData(account)
     * @param currentPeriodStartDebtIndex The startingDebtIndex of the current fee period
     * @dev onlyFeePool to call me on dpassive.issue() & dpassive.burn() calls to store the locked DPS
    ...
    /**
     * @notice Import issuer data from dpassiveState.issuerData on FeePeriodClose() block #
