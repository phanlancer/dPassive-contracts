...
contract RewardsDistribution is Owned, IRewardsDistribution {
    ...
    /**
     * @notice Address of the DPassive ProxyERC20
     */
    address public dpassiveProxy;
    ...
    /**
     * @dev _authority maybe the underlying dpassive contract.
     * Remember to set the authority on a dpassive upgrade
     */
    constructor(
        address _owner,
        address _authority,
        address _dpassiveProxy,
        address _rewardEscrow,
        address _feePoolProxy
    ) public Owned(_owner) {
        authority = _authority;
        dpassiveProxy = _dpassiveProxy;
    ...
    function setDPassiveProxy(address _dpassiveProxy) external onlyOwner {
        dpassiveProxy = _dpassiveProxy;
    }
    ...
    function distributeRewards(uint amount) external override returns (bool) {
        ...
        require(dpassiveProxy != address(0), "DPassiveProxy is not set");
        require(feePoolProxy != address(0), "FeePoolProxy is not set");
        require(
            IERC20(dpassiveProxy).balanceOf(address(this)) >= amount,
        ...
        for (uint i = 0; i < distributions.length; i++) {
            if (distributions[i].destination != address(0) || distributions[i].amount != 0) {
                ...
                // Transfer the DPS
                IERC20(dpassiveProxy).transfer(distributions[i].destination, distributions[i].amount);

                // If the contract implements RewardsDistributionRecipient.sol, inform it how many DPS its received.
        ...
        IERC20(dpassiveProxy).transfer(rewardEscrow, remainder);
