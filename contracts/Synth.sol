...
contract Synth is Owned, ExternStateToken, MixinResolver, ISynth {
    ...
    // Currency key which identifies this Synth to the DPassive system
    ...
    function transfer(address to, uint value) public optionalProxy returns (bool) {
        ...
        // transfers to FEE_ADDRESS will be exchanged into dUSD and recorded as fee
        if (to == FEE_ADDRESS) {
    ...
    function transferAndSettle(address to, uint value) public override optionalProxy returns (bool) {
     ...
     * non-dUSD synths are exchanged into dUSD via synthInitiatedExchange
     * notify feePool to record amount as fee paid to feePool */
    function _transferToFeeAddress(address to, uint value) internal returns (bool) {
        uint amountInUSD;

        // dUSD can be transferred to FEE_ADDRESS directly
        if (currencyKey == "dUSD") {
            amountInUSD = value;
            super._internalTransfer(messageSender, to, value);
        } else {
            // else exchange synth into dUSD and send to FEE_ADDRESS
            amountInUSD = exchanger().exchange(messageSender, currencyKey, value, "dUSD", FEE_ADDRESS);
        }

        // Notify feePool to record dUSD to distribute as fees
        feePool().recordFeePaid(amountInUSD);

        return true;
    }

    function issue(address account, uint amount) external virtual override onlyInternalContracts {
        _internalIssue(account, amount);
    }

    function burn(address account, uint amount) external virtual override onlyInternalContracts {
