...
abstract contract VirtualSynth is ERC20, IVirtualSynth {
    ...
    function rate() external view override returns (uint) {
        return calcRate();
    }
    ...
    function secsLeftInWaitingPeriod() external view override returns (uint) {
        return secsLeft();
    }

    function readyToSettle() external view override returns (bool) {
        return secsLeft() == 0;
    }
