...
contract Depot is Owned, Pausable, ReentrancyGuard, MixinResolver, IDepot {
    ...
    // Address where the ether and Synths raised for selling DPS is transfered to
    ...
    /* This is a convenience variable so users and dApps can just query how much dUSD
    ...
    uint public override totalSellableDeposits;

    // The minimum amount of dUSD required to enter the FiFo queue
    uint public override minimumDepositAmount = 50 * SafeDecimalMath.unit();

    // A cap on the amount of dUSD you can buy with ETH in 1 transaction
    uint public override maxEthPurchase = 500 * SafeDecimalMath.unit();
    ...
    /**
     * @notice Set the minimum deposit amount required to depoist dUSD into the FIFO queue
     * @param _amount The new new minimum number of dUSD required to deposit
     */
    ...
    /**
     * @notice Fallback function (exchanges ETH to dUSD)
     */
    ...
    function exchangeEtherForSynths()
        ...
        notPaused
        returns (
            uint // Returns the number of Synths (dUSD) received
        )
    {
    ...
    function _exchangeEtherForSynths() internal returns (uint) {
        ...
        for (uint i = depositStartIndex; remainingToFulfill > 0 && i < depositEndIndex; i++) {
            ...
            if (deposit.user == address(0)) {
                ...
                if (deposit.amount > remainingToFulfill) {
                    ...
                    // dpassive foundation funds wallet. This is to protect all depositors
                    ...
                    synthdUSD().transfer(msg.sender, remainingToFulfill);
                    ...
                    //       we request a specific transfer here, the fee is
                    //       automatically deducted and sent to the fee pool.
                    synthdUSD().transfer(msg.sender, deposit.amount);
        ...
        if (fulfilled > 0) {
            ...
            emit Exchange("ETH", msg.value, "dUSD", fulfilled);
        }
    ...
    /**
     * @notice Exchange ETH to dUSD while insisting on a particular rate. This allows a user to
    ...
    function _exchangeEtherForDPS() internal returns (uint) {
        // How many DPS are they going to be receiving?
        uint dpassiveToSend = dpassiveReceivedForEther(msg.value);

        // Store the ETH in our funds wallet
        fundsWallet.transfer(msg.value);

        // And send them the DPS.
        dpassive().transfer(msg.sender, dpassiveToSend);

        emit Exchange("ETH", msg.value, "DPS", dpassiveToSend);

        return dpassiveToSend;
    }

    /**
     * @notice Exchange ETH to DPS.
     */
    function exchangeEtherForDPS()
        external
        payable
        override
        rateNotInvalid(DPS)
        ...
        notPaused
        returns (
            uint // Returns the number of DPS received
        )
    {
        return _exchangeEtherForDPS();
    }

    /**
     * @notice Exchange ETH to DPS while insisting on a particular set of rates. This allows a user to
     *         exchange while protecting against frontrunning by the contract owner on the exchange rates.
     * @param guaranteedEtherRate The ether exchange rate which must be honored or the call will revert.
     * @param guaranteedDPassiveRate The dpassive exchange rate which must be honored or the call will revert.
     */
    function exchangeEtherForDPSAtRate(uint guaranteedEtherRate, uint guaranteedDPassiveRate)
        external
        payable
        override
        rateNotInvalid(DPS)
        ...
        )
    {
        ...
        require(
            guaranteedDPassiveRate == exchangeRates().rateForCurrency(DPS),
    ...
    function _exchangeSynthsForDPS(uint synthAmount) internal returns (uint) {
        // How many DPS are they going to be receiving?
        uint dpassiveToSend = dpassiveReceivedForSynths(synthAmount);
        ...
        emit Exchange("dUSD", synthAmount, "DPS", dpassiveToSend);

        return dpassiveToSend;
    }

    /**
     * @notice Exchange dUSD for DPS
     * @param synthAmount The amount of synths the user wishes to exchange.
     */
    function exchangeSynthsForDPS(uint synthAmount)
        external
        override
        rateNotInvalid(DPS)
        notPaused
    ...
    {
        return _exchangeSynthsForDPS(synthAmount);
    }

    /**
     * @notice Exchange dUSD for DPS while insisting on a particular rate. This allows a user to
     *         exchange while protecting against frontrunning by the contract owner on the exchange rate.
     * @param synthAmount The amount of synths the user wishes to exchange.
     * @param guaranteedRate A rate (DPS price) the caller wishes to insist upon.
     */
    function exchangeSynthsForDPSAtRate(uint synthAmount, uint guaranteedRate)
        external
        rateNotInvalid(DPS)
        notPaused
        returns (
    ...
    {
        require(guaranteedRate == exchangeRates().rateForCurrency(DPS), "Guaranteed rate would not be received");

        return _exchangeSynthsForDPS(synthAmount);
    }

    /**
     * @notice Allows the owner to withdraw DPS from this contract if needed.
     * @param amount The amount of DPS to attempt to withdraw (in 18 decimal places).
     */
    function withdrawDPassive(uint amount) external override onlyOwner {
        dpassive().transfer(owner, amount);

        // We don't emit our own events here because we assume that anyone
        // who wants to watch what the Depot is doing can
        // just watch ERC20 events from the Synth and/or DPassive contracts
    ...
    function withdrawMyDepositedSynths() external override {
        ...
        synthdUSD().transfer(msg.sender, synthsToSend);
     ...
     * @param amount The amount of dUSD you wish to deposit (must have been approved first)
     */
    function depositSynths(uint amount) external override {
        // Grab the amount of synths. Will fail if not approved first
        synthdUSD().transferFrom(msg.sender, address(this), amount);
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        addresses[0] = CONTRACT_SYNTHDUSD;
        addresses[1] = CONTRACT_EXRATES;
        addresses[2] = CONTRACT_DPASSIVE;
    }

    /**
     * @notice Calculate how many DPS you will receive if you transfer
     *         an amount of synths.
     * @param amount The amount of synths (in 18 decimal places) you want to ask about
     */
    function dpassiveReceivedForSynths(uint amount) public view override returns (uint) {
        // And what would that be worth in DPS based on the current price?
        return amount.divideDecimal(exchangeRates().rateForCurrency(DPS));
    }
     ...
     */
    function dpassiveReceivedForEther(uint amount) public view override returns (uint) {
        // How much is the ETH they sent us worth in dUSD (ignoring the transfer fee)?
        uint valueSentInSynths = amount.multiplyDecimal(exchangeRates().rateForCurrency(ETH));

        // Now, how many DPS will that USD amount buy?
        return dpassiveReceivedForSynths(valueSentInSynths);
    }
     ...
     * @param amount The amount of ether (in wei) you want to ask about
     */
    function synthsReceivedForEther(uint amount) public view override returns (uint) {
    ...
    function synthdUSD() internal view returns (IERC20) {
        return IERC20(requireAndGetAddress(CONTRACT_SYNTHDUSD));
    }

    function dpassive() internal view returns (IERC20) {
        return IERC20(requireAndGetAddress(CONTRACT_DPASSIVE));
    }
