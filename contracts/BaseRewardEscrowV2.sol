...
contract BaseRewardEscrowV2 is Owned, IRewardEscrowV2, LimitedSetup(8 weeks), MixinResolver {
    ...
    /* An account's total escrowed DPS balance to save recomputing this for fee extraction purposes. */
    mapping(address => uint256) public override totalEscrowedAccountBalance;

    /* An account's total vested reward DPS. */
    ...
    /* The total remaining escrowed balance, for verifying the actual DPS balance of this contract against. */
    ...
    function getVestingSchedules(
        ...
        uint256 index,
        uint256 pageSize
    ) external view override returns (VestingEntries.VestingEntryWithID[] memory) {
    ...
    function getVestingQuantity(address account, uint256[] calldata entryIDs) external view override returns (uint total) {
    ...
    function getVestingEntryClaimable(address account, uint256 entryID) external view override returns (uint) {
        VestingEntries.VestingEntry memory entry = vestingSchedules[account][entryID];
    ...
    /**
     * @notice Create an escrow entry to lock DPS for a given duration in seconds
    ...
    function createEscrowEntry(
        ...
        /* Transfer DPS from msg.sender */
        require(IERC20(address(dpassive())).transferFrom(msg.sender, address(this), deposit), "token transfer failed");
     ...
     * @dev A call to this should accompany a previous successful call to dpassive.transfer(rewardEscrow, amount),
     * to ensure that when the funds are withdrawn, there is enough balance.
     * @param account The account to append a new vesting entry to.
     * @param quantity The quantity of DPS that will be escrowed.
     * @param duration The duration that DPS will be emitted.
     */
    function appendVestingEntry(
        address account,
        uint256 quantity,
        uint256 duration
    ) external override onlyFeePool {
    ...
    function _transferVestedTokens(address _account, uint256 _amount) internal {
        ...
        IERC20(address(dpassive())).transfer(_account, _amount);
    ...
    function accountMergingIsOpen() public view override returns (bool) {
        return accountMergingStartTime.add(accountMergingDuration) > block.timestamp;
    }

    function startMergingWindow() external override onlyOwner {
    ...
    function nominateAccountToMerge(address account) external override {
        require(account != msg.sender, "Cannot nominate own account to merge");
        require(accountMergingIsOpen(), "Account merging has ended");
        require(issuer().debtBalanceOf(msg.sender, "dUSD") == 0, "Cannot merge accounts with debt");
        nominatedReceiver[msg.sender] = account;
        emit NominateAccountToMerge(msg.sender, account);
    }

    function mergeAccount(address accountToMerge, uint256[] calldata entryIDs) external override {
        require(accountMergingIsOpen(), "Account merging has ended");
        require(issuer().debtBalanceOf(accountToMerge, "dUSD") == 0, "Cannot merge accounts with debt");
    ...
    function migrateVestingSchedule(address) external virtual override {
        _notImplemented();
    }
    ...
    function _appendVestingEntry(
        ...
        require(
            totalEscrowedBalance <= IERC20(address(dpassive())).balanceOf(address(this)),
