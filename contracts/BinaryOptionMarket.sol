...
contract BinaryOptionMarket is Owned, MixinResolver, IBinaryOptionMarket {
    ...
    constructor(
        ...
        // Instantiate the _options themselves
        _options.long = new BinaryOption(_creator, longBid);
        _options.short = new BinaryOption(_creator, shortBid);
    }
    ...
    function senderPriceAndExercisableDeposits() external view returns (uint price, uint exercisable) {
        ...
        if (msg.sender == address(_options.long)) {
            price = prices.long;
        } else if (msg.sender == address(_options.short)) {
            price = prices.short;
        } else {
    ...
    function pricesAfterBidOrRefund(
        ...
        if (side == Side.Long) {
            longTotalBids = refund ? longTotalBids.sub(value) : longTotalBids.sub(value);
        } else {
            shortTotalBids = refund ? shortTotalBids.sub(value) : shortTotalBids.add(value);
        }

        if (refund) {
            value = value.multiplyDecimalRound(SafeDecimalMath.unit().sub(fees.refundFee));
        }
        return _computePrices(longTotalBids, shortTotalBids, refund ? deposited.sub(value) : deposited.add(value));
    }
    ...
    function _bidsOf(address account) internal view returns (uint long, uint short) {
        return (_options.long.bidOf(account), _options.short.bidOf(account));
    }
    ...
    function _totalBids() internal view returns (uint long, uint short) {
        return (_options.long.totalBids(), _options.short.totalBids());
    }
    ...
    function _claimableBalancesOf(address account) internal view returns (uint long, uint short) {
        return (_options.long.claimableBalanceOf(account), _options.short.claimableBalanceOf(account));
    }
    ...
    function totalClaimableSupplies() external view override returns (uint long, uint short) {
        return (_options.long.totalClaimableSupply(), _options.short.totalClaimableSupply());
    }

    function _balancesOf(address account) internal view returns (uint long, uint short) {
        return (_options.long.balanceOf(account), _options.short.balanceOf(account));
    }
    ...
    function totalSupplies() external view override returns (uint long, uint short) {
        return (_options.long.totalSupply(), _options.short.totalSupply());
    }
    ...
    function _option(Side side) internal view returns (BinaryOption) {
        if (side == Side.Long) {
            return _options.long;
        }
        return _options.short;
    }
        ...
        returns (uint longClaimed, uint shortClaimed)
    {
        ...
        // Only claim _options if we aren't resolved, and only claim the winning side.
        uint longOptions;
        uint shortOptions;
        if (!_resolved || outcome == Side.Long) {
            longOptions = _options.long.claim(msg.sender, prices.long, exercisable);
        }
        if (!_resolved || outcome == Side.Short) {
            shortOptions = _options.short.claim(msg.sender, prices.short, exercisable);
        }
    ...
    function exerciseOptions() external override returns (uint) {
        ...
        if (longBalance != 0) {
            _options.long.exercise(msg.sender);
        }
        if (shortBalance != 0) {
            _options.short.exercise(msg.sender);
        }
    ...
    function _selfDestruct(address payable beneficiary) internal {
        ...
        _options.long.expire(beneficiary);
        _options.short.expire(beneficiary);
