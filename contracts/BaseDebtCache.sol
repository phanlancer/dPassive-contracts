...
contract BaseDebtCache is Owned, MixinSystemSettings, IDebtCache {
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[5] = CONTRACT_ETHERCOLLATERAL_DUSD;
    ...
    function cachedDebt() external view override returns (uint) {
        return _cachedDebt;
    }

    function cachedSynthDebt(bytes32 currencyKey) external view override returns (uint) {
        return _cachedSynthDebt[currencyKey];
    }

    function cacheTimestamp() external view override returns (uint) {
        return _cacheTimestamp;
    }

    function cacheInvalid() external view override returns (bool) {
    ...
    function cacheStale() external view override returns (bool) {
        ...
        )
    {
        ...
        (uint excludedDebt, bool isAnyNonDpsDebtRateInvalid) = _totalNonDpsBackedDebt();
        return (values, excludedDebt, isInvalid || isAnyNonDpsDebtRateInvalid);
    }
    ...
    function cachedSynthDebts(bytes32[] calldata currencyKeys)
        ...
        view
        override
        returns (uint[] memory dpsIssuedDebts)
    {
        return _cachedSynthDebts(currencyKeys);
    }

    // Returns the total dUSD debt backed by non-DPS collateral.
    function totalNonDpsBackedDebt() external view override returns (uint excludedDebt, bool isInvalid) {
        return _totalNonDpsBackedDebt();
    }

    function _totalNonDpsBackedDebt() internal view returns (uint excludedDebt, bool isInvalid) {
        // Calculate excluded debt.
        // 1. Ether Collateral.
        excludedDebt = excludedDebt.add(etherCollateraldUSD().totalIssuedSynths()); // Ether-backed dUSD
        ...
        if (etherCollateralTotalIssuedSynths > 0) {
            (uint dETHRate, bool dETHRateIsInvalid) = exchangeRates().rateAndInvalid(dETH);
            isInvalid = isInvalid || dETHRateIsInvalid;
            excludedDebt = excludedDebt.add(etherCollateralTotalIssuedSynths.multiplyDecimalRound(dETHRate)); // Ether-backed dETH
        }
    ...
    function _currentDebt() internal view returns (uint debt, bool anyRateIsInvalid) {
        ...
        return (total, isInvalid || isAnyNonDpsDebtRateInvalid);
    }

    function currentDebt() external view override returns (uint debt, bool anyRateIsInvalid) {
        return _currentDebt();
    }
